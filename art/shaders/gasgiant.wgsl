...
/// 把世界方向转回条带图那个**未倾斜的局部系**，再采一次条带场。
fn band_of(direction: vec3<f32>) -> f32 {
    let local = rotate_vector(vec4<f32>(-params.orientation.xyz, params.orientation.w), direction);
...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    var band = clamp(band_of(normal), 0.0, 1.0);
