//! **NURBS 域**的数据那一半：曲线 / 曲面 / 求值读数，以及十三个 NURBS 算子的参数与
//! **声明**（`ops.rs`：`px_op!` 那几行）。
//!
...
//!   圆就不再是圆），而"精确圆"正是 NURBS 存在的理由。要进网格 / 折线时才降到 `f32`
//!   （细分那一步）。
...
// 细分那一档的两个产物：曲面 → 网格、曲线 → 折线。它们住在契约层
// （`px_protocol::art`，线格式那一层），这里 re-export 一次，好让图脚本一行就够。
