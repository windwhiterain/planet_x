...
export const TUNING = {
  ...
  // 小行星带 / 柯伊伯带：实例化碎岩。0 = 关。
  beltDensity: 1.0,
  // 日珥的**喷发周期**（秒，基准值；每条带子会按 `aFlow2.w` 在 0.6~1.7× 之间自己取）。
  // 相位来自一个平缓的世界空间场 ⇒ 相邻带子相位接近，**一片一片地喷**（见 sunfield.js）。
  // 截图会冻结时间轴（`?t=`），所以这个值不影响可复现性。
  promPeriod: 34.0,
  // 日珥的**顶点驱动位移**幅度（世界单位）：每个顶点沿"自己位置上的场 − 根部场"位移。
  // 这就是"扭"的来源（微分量：跨宽度那部分让截面转，跨长度那部分让它顺场弯，二阶的是剪切）。
  // 实测转角 ≈ ½·|ω|·amp：0.30 ⇒ ~0.84 rad（约 48°）。0 = 关（退回刚性带子）。
  promDisp: 0.30,
