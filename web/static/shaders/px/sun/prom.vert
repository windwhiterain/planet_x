...
#include <px/noise/perlin.glsl>
  ...
  void main(){
    ...
    //   取差之后剩下的全是**差动**：跨宽度的那份 ⇒ 截面转（扭）、跨长度的那份 ⇒ 顺场弯、
    //   二阶的那份 ⇒ 剪切。
    //
    // ⚠⚠ **但"取差之后根部就为 0"是错的**（上一版我就是这么写错的，用户报了
    //   「有带子的根部浮空的现象」）。两个原因：
    //   ① `aField0` 烘在 `aDir*sunR`，而带子的**根部其实在 `up` 方向**上 ——
    //      `up = normalize(aDir + k0*tilt)`，`tilt` 典型 ~0.5 rad（30°）：基准点跑到
    //      30° 之外，那里的场当然不一样 ⇒ 差动不为 0，位移的**径向分量**把根抬离球面
    //      （量级 |F|×uDisp ≈ 0.22 世界单位 ≈ 0.034R ≈ 日面视角 9 px）。
    //      修法在 CPU 侧：`aField0` 改成烘在**实际的根部点**上（见 sun.js）。
    //   ② 就算基准点对上了，根部**一条线**（x = ±0.5）的两端与"根部中心"的场也不同，
    //      而且拱的另一只脚（t=1）同样需要锚住。
    //   ⇒ 所以这里再乘一个**渐入/渐出**：根部 0、尖端（拱的另一只脚）也 0。
    //      物理上也对：光球把磁流管锚在日面上，位移本来就该从锚点长出来。
    float fade = smoothstep(0.0, 0.30, t) * (1.0 - arc * smoothstep(0.70, 1.0, t));
    vec3 off = (promFieldAt(pBase) - aField0) * (uDisp * fade);
    vec3 p = pBase + off;
