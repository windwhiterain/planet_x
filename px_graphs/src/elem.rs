...
pub fn source_hash_of<F: ElementFn>() -> Result<&'static str, px_graph_schema::Fault> {
    static KEYS: std::sync::Mutex<Vec<(std::any::TypeId, &'static str)>> =
        std::sync::Mutex::new(Vec::new());
    let id = std::any::TypeId::of::<F>();
    let mut keys = KEYS.lock().map_err(px_graph_schema::ops::lock_error)?;
...
pub fn render<F: ElementFn>(params: &F::Params, inputs: &F::Inputs) -> Result<Field, String> {
    let path = px_cook::inst::library_path(source_hash_of::<F>().map_err(String::from)?);
    let body =
...
impl<F: ElementFn> px_graph_schema::PxOp for ElemOp<F> {
    ...
    fn source_hash() -> Result<&'static str, px_graph_schema::Fault> {
        source_hash_of::<F>()
    }
