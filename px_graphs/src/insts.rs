...
//!   **没有生成物**，它的规格是普通 Rust 代码（`ELEM_SPECS`）⇒ 本模块多出"照表登记节点与
//!   catalogue 条目"的一段（`build` / `codegen` 里各一个循环）。两档的**身份还是同一个函数**
//!   算的（`key_of_facts`），只是表从"生成物里那串字面量"换成了"编译进图程序的常量表"。
...
///   生成器把它们写成了 const 字面量。
/// ⚠ **element 那一档没有生成物**（内容键在运行期算，见 `px_elem` 的模块文档）：它的规格
