...
- ship_orders：本方各舰的 行为（ShipBehavior）：Idle（待命）、Move{position}（前往某位置）、\n\
  ...
  TargetSettlement{city,bombard}（围攻某城）、Dock{body}（停泊：跟随某天体）、None（原地不动）。\n\
...
/// High-level ship command: `order <ship> attack|chase|guard|siege|move|dock|colonize|none|idle ...`.
...
fn cmd_order(state: &mut State, config: &GameConfig, rest: &str) -> Result<(), String> {
    ...
    let ship: u32 = t.next().and_then(|s| s.parse().ok()).ok_or("usage: order <ship> attack|chase|guard|siege|move|dock|colonize|none|idle ...")?;
...
fn guide_json() -> String {
    json!({
        ...
        "commands": {
            ...
            "order": {"usage": "order <ship> attack|chase|guard|siege|move|dock|colonize|none|idle ...", "desc": "one-shot ship command (Player mode). attack <enemy> chases+fires; guard <friend> escorts; dock <body> follows a body; none holds position. e.g. order 0 attack 2 | order 3 dock 9 | order 8 none"},
