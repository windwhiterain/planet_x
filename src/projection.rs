        ...
        let entry = match t.name {
            ...
            "round_inputs" => json!({
                ...
                "column_docs": {
                    ...
                    "rolls": "**`derived_roll` 家族的抽签记录**：每条 = `{purpose, faction, subject, value, threshold, pool_total, picked}`。**三种形状**：**闸门**（`threshold` 有值、`pool_total` 为空，判据是 `value < threshold`，`picked` 是走的那一支）、**加权抽签**（`pool_total` 有值、`threshold` 为空，`value × pool_total` 落在哪一段，`picked` 是选中的那一段）、**幅度骰**（两者都空 = 掷出的数直接被当成量用：只有 `nav`，`picked` 是算出来的落点）。已接的用途（B5b）：定编 `role` / `observe_role`、派单 `route`、合同 `gate`（**为什么没人接我的单**：`heard`/`unheard`） / `accept` / `pick` / `assign` / `quit` / `review` / `renew`、船坞 `retool`（`war-retool` 与 `hauler-retool` 两类，`subject` 区分） / `blueprint_intent` / `blueprint_retune` / `blueprint_theme`、风格轴 `style_chance`（逐舰逐轴）、导航 `nav`（MOND 偏航）。⚠ 这些骰子**不消费主 `Prng`**（`(势力, 对象, 回合, 用途)` 的哈希 ⇒ 可重算），但**判据不可重算**（候选池/权重/机会值都是那一刻的）——所以两者都留。⚠ **只在「拍板处」记一条**：同一枚骰子会被「估算」与「拍板」问两次（例如挂单时估运力也问 role），记的是**决定**不是「谁算过」；没掷的档（整期无产出不续约、缺口为 0 不掷重构、承包单指定路线）**不记**。⚠ **量**：实测 seed 7 / 30 回合 **3944 条 ≈ 127 条/回合 ≈ 21.6 KB/回合**（`main.jsonl` 是 26.2 KB/回合 ⇒ 这一面已与整份视图同量级），其中 `style_chance`（逐舰逐轴）与合同 `gate`（逐势力逐单）两项占一半。",
                },
            }),
            "decisions" => json!({
                "table": t.table, "key": t.key, "join_on": t.join_on, "round": t.round,
