        ...
        let entry = match t.name {
            ...
            "round_inputs" => json!({
                ...
                "column_docs": {
                    ...
                    "role_distribution": "**本回合定编的配给、抽签与实得**（`{势力: {war/freight/observe: {quota, held, gap_join, gap_leave, rotation, flow_join, flow_leave, sum_p_join, sum_p_leave, clamped_join, clamped_leave, fixed, actual, exogenous}}}`，`autocontrol::freight::assign_roles` 填）：`quota` = 目标头数、`held` = 本回合开始时这一支的在册头数、`gap_*` = `max(0, quota − held)` 与其反面、`rotation` = 本支轮换率、`flow_*` = **引擎自己算的期望**（`缺口 + 轮换 × 头数`）、`sum_p_*` = 池子里那一侧各舰机会值之和、`clamped_*` = 被 `min(1,·)` 截断的舰数、`fixed` = **不掷骰**就定性的头数（承包单 / 舱里有货 / 玩家表态 / 动不了 / 没配额）、`actual` / `exogenous`。**判据**：池子只有一侧 ⇒ `Σp` 应当正好等于 `flow`；实测 **22852/25158 行相等、2306 行 `Σp < flow`**（引擎「同侧总票数」与「在册头数」的口径不一致，原因待定，见 `.agents/notes/test-migration-backlog.md`）⇒ 目前只压单向界 `Σp ≤ flow`。与 `rolls` 同一条口径：**没定编就是没有**（空对象）。",
