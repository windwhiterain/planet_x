...
fn upkeep_shortfall_records_the_unpaid_part_and_the_rust_it_causes() {
    ...
    for (name, hmax) in &hull_max {
        let s = state.ships.iter().find(|s| &s.name == name).expect("舰还在（只锈了一半）");
    ...
    assert!(row.fleet_rust > 0.0, "掉血必须在读面上看得见——锈到 0 之前没有任何事件");
}
...
fn spent_never_exceeds_the_batch_and_the_gap_is_the_unspent_part() {
    ...
    for f in &state.factions {
        ...
        for (kind, spent, limits) in [
            ("investment_budget", &row.investment_spent, &ctl.investment_budget),
            ("construction_budget", &row.construction_spent, &ctl.construction_budget),
        ] {
            for (rt, amt) in spent {
            ...
            for (rt, leaf) in limits.iter() {
                ...
                assert!(leaf.value - amt >= -1e-9, "{fid} 的 {kind}.{rt} 出现了负余额");
    ...
    assert!(unspent_seen >= 1, "所有额度都被花光了——「批了没花掉」这条读法没被检查到");
}
...
fn build_lines_separate_the_money_bottleneck_from_the_capacity_ceiling() {
    ...
    let (cid, bid, class) = state
        ...
        .find_map(|c| {
            c.buildings
                ...
                .map(|b| (c.name.clone(), b.id, b.ship_type.clone().unwrap_or_else(|| "护卫舰".to_string())))
        })
        .expect("中国开局应有建造区");
    ...
    let poor_line = poor
        ...
        .get(&cid)
        .and_then(|f| f.build.get(&class))
        .unwrap_or_else(|| panic!("批 0 时 {cid} 的建造行**仍然必须在**（那是「没钱」而不是「没船坞」）"))
        .clone();
    ...
    assert!(
        poor.spend.get(&fid).map(|s| s.investment.is_empty()).unwrap_or(false)
            || poor.spend.get(&fid).map(|s| s.construction.values().all(|v| *v <= 0.0)).unwrap_or(false),
...
fn labor_and_housing_capacity_are_captured_from_this_steps_state() {
    ...
    let cf = sink.city_flow.get(&cid).expect("step_production 必须记这一格");
    assert!(
    ...
    let (ecocap, want_housing) = {
        let ecocap = state.city_settlement(&cid).map(|s| s.ecological_capacity).unwrap_or(0.0);
        let area: f64 = state
            .city(&cid)
            .expect("刚查过")
            .buildings
    ...
    assert!(cf.housing_capacity > 0.0, "{cid} 开局应有住宅（否则这条对账没意义）");
    assert!(
