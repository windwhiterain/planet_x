...
fn the_carrier_keeps_its_share_of_each_delivery() {
    let c = sample();
    assert!(
        (c.carrier_cut(10.0) - 1.5).abs() < 1e-9,
        "15% 抽成 ⇒ 10 件里自留 1.5"
    );
    assert!(
        (10.0 - c.carrier_cut(10.0) - 8.5).abs() < 1e-9,
        "余数 8.5 进雇主首都池"
    );
    ...
    wild.share = 3.0;
    assert!(
        (wild.carrier_cut(10.0) - 10.0).abs() < 1e-9,
        "比例被夹到 1.0"
    );
}
...
fn there_is_at_most_one_open_order_per_lane() {
    ...
    assert!(
        cs.open_mut("美国", "金星", "地球").is_none(),
        "别人的单不算我的"
    );
    assert!(
        cs.open_mut("中国", "水星", "地球").is_none(),
        "别的货栈各自成单"
    );
    // 有人接了 ⇒ 不再可改（承诺冻结）。
