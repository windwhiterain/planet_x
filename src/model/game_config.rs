...
pub struct FreightConfig {
    /// **开叫价**：承运人的抽成比例（0.15 = 交付时从货里自留 15%，其余进托运方首都池）。
    /// 它是 Q10 的机制落点：**没有货币转移**，报酬就是承运人没交出去的那部分货。
    ///
    /// 这只是**起叫**：没人接的单子会一路抬价（见 [`Self::share_escalation`] / [`Self::share_max`]），
    /// 所以「难单该报多厚」不由配置决定，而由市场自己找出来（用户裁决：价格做成动态平衡）。
    pub share: f64,
    /// **加价档**：一张没人接的单子过了自己报的截止期之后，抽成乘这个系数（1.12 = 每次 +12%）。
    ///
    /// 这是「难单自然报厚」的机制：近地小单开叫就被抢走（价格留在开叫价），深空/大单
    /// 没人接 ⇒ 价格一路抬到有人点头。**上限**见 [`Self::share_max`]。
    pub share_escalation: f64,
    /// **抽成上限**：抬价抬到这里就不再抬（0.5 = 最多把一半的货给承运人）。
    /// 过了这条线还送不出去，托运方宁可让货烂在产地——这也是给「自己造船运」留的价值。
    pub share_max: f64,
    ...
    /// 风险（超期掉信誉），不是漏洞。**托运方的信誉门槛**也用它（同一个道理）。
    pub reference_speed: f64,
    /// 挂单时**假定的承运人舱容**（单位/趟）：用来把「要搬多少」折成**趟数**。
    ///
    /// 必须按趟数算，因为实测平均单量 **47 件**而最大的货舱是 20（航母）、常见的是 4/6
    /// ——**每一张单都是多趟的活**。取舰队里最常见的舱容（驱逐 4 / 巡洋 6 / 战列 6），
    /// **不取航母的 20**：按 20 算会让截止期短到几乎没人做得到（实测那样做时 35 张成交
    /// 里 21 张超期）。
    pub nominal_hold: f64,
    /// **门槛基准**：难度为 0、货值近乎为 0 的单，也要的最低信誉。
    /// 门槛 = `gate_base + gate_slope × (难度 + 货值份数)`，见 `autocontrol::contract`。
    pub gate_base: f64,
    /// 门槛对「难度 + 货值份数」的斜率。
    pub gate_slope: f64,
    ...
    /// 超期的信誉扣减（**只扣一次**，Q11：合同不作废、货不消失）。
    pub reputation_late_penalty: f64,
    /// 接单后没送到（船毁/放弃）的信誉扣减。
    pub reputation_lost_penalty: f64,
...
impl Default for FreightConfig {
    fn default() -> Self {
        Self {
            ...
            reputation_late_penalty: 0.15,
            reputation_lost_penalty: 0.35,
            reputation_max: 5.0,
        }
    }
}
