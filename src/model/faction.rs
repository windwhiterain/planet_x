...
pub struct Faction {
    ...
    ///
    /// 开局值来自 `config.mond.initial`（**现在是空表：没人白拿**，见用户裁决「特权删掉」）；
    /// 之后由 `sim::step_knowledge` 按**飞船在异常区**的在场强度驱动
    /// （用户裁决：先只做这一条渠道）。**学到顶（1.0）就永久持有**（棘轮）；
    /// 1.0 之下会随「不在场」慢慢锈回凡人。`serde(default)` = 0 让旧档退化成凡人。
    #[serde(default)]
