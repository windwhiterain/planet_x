...
///
/// ⚠ **逐舰机会值（"分布"本身）还没记全**：`rolls` 里只有**落中那一支**的 `p`，而落空那一支
/// 的机会值只在 `assign_roles` 里存在过（观测没命中的舰仍掷过观测的骰子）。想把
/// `Σp == 缺口 (+ 轮换×在册)` 做成逐字对账的恒等式，得**同时**把引擎自己的分账
/// （同侧候选票之和 `tickets`、同侧可用头数 `others`）交出来——只记 `p` 的话对不上：
/// 实测 `北斗`（r2 中国）闸门 `threshold = 0.0`，而同一艘舰的 `flow = 0.05`（`others` 只算
/// "掷得动的舰"，而两支的**头数**算的是全部在册）⇒ 差在**分母/头数的口径**，不在公式。
///
/// 这也是"直接测分布而不是采样"那件事**还差的一步**：见
/// `.agents/notes/test-migration-backlog.md`。
#[derive(Serialize, Deserialize, Clone, Debug, Default, PartialEq, schemars::JsonSchema)]
