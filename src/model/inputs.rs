...
/// 某一支（战舰 / 运输 / 观测）本回合的**配给与实得**。
///
/// 定编是**级联**的：观测（优先级 1，自己一枚骰子）→ 运输（优先级 2）→ 战舰（兜底），
/// 而硬承诺 / 舱里有货 / 玩家表态 / 动不了 这几档**不掷骰**（确定性）。这里只记**能直接辩护**
/// 的那几个数：配给给了多少、抽签前手里有多少、多少艘根本没掷骰、最后实得多少。
///
/// **逐舰机会值现在记全了**（两枚骰子各带自己的 `flow`：同一艘舰会先后掷观测与运输两枚，
/// 共用一个字段会被后一枚覆盖——第一版就是这么错的）。但实测 `Σp == flow` **只在大约 91% 的
/// 行上成立**（7 seed × 1000 回合：25158 行里 22852 行相等、2306 行 `Σp < flow`）：
/// **池子里摊了票、而那些票没被记成机会值** ⇒ 引擎的"同侧总票数"与"在册头数"有一处口径不一致。
///
/// ⚠ **原因还没定**，别照抄想当然的答案：我一度以为"订单叶被玩家钉死的舰还在分母里"，
/// 用 `ships.order_leaf_mode` 判时看着 103 行全对上；改用引擎真正读的
/// `ships.order_effective_mode` 之后那一类变成 **0 行** ⇒ 与玩家钉死**无关**。
/// 下一步要记**池子人数与票数**（`pool_n` / `tickets`）才能定因。
#[derive(Serialize, Deserialize, Clone, Debug, Default, PartialEq, schemars::JsonSchema)]
pub struct RoleShare {
    /// [`crate::autocontrol::freight::role_quotas`] 给的**目标头数**（连续量）。
    pub quota: f64,
    /// 本回合**开始**时这一支的**在册头数**（在任 + 硬承诺 + 外生），即配额要服务的对象。
    pub held: f64,
    /// 入伙侧的缺口 `max(0, quota − held)`；退伍侧的超额 `max(0, held − quota)`。
    pub gap_join: f64,
    pub gap_leave: f64,
    ...
    /// 本回合**实得**头数。
    pub actual: f64,
    /// **外生**头数：轮不到自动控制决定的活舰（玩家的叶钉死 / 订单叶不是 Auto）。
    pub exogenous: f64,
}
