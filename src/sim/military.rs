...
pub fn step_military(state: &mut State, config: &GameConfig, rng: &mut Prng, flow: &mut RoundSink) {
    ...
    for ship_id in order {
        ...
        if !is_ai {
            ...
            let mut behavior = state
                .ship_behavior(ship_id.clone())
                .unwrap_or(ShipBehavior::Idle);
            ...
            if !behavior_is_valid(state, config, behavior.clone(), &owner) {
                let reason = match behavior {
                    ...
                    ShipBehavior::Colonize { body } => {
                        format!("body {body} has no blank settlement")
                    }
                ...
                if let Some(c) = state.control_mut(owner.clone()) {
                    c.ship_orders
                        .insert(ship_id.clone(), Control::player(ShipBehavior::Idle));
                }
                ev(
                    state,
    ...
    for (i, s) in state.ships.iter_mut().enumerate() {
        if s.hull > 0.0 {
            ...
            s.hull =
                (s.hull + panel.hull_max * (panel.hull_regen + bonuses[i])).min(panel.hull_max);
...
pub fn bombard_city(state: &mut State, config: &GameConfig, ship_id: &str, cid: &str) {
    ...
    let razed = {
        ...
        for b in &mut c.buildings {
            let share = if total_deployed > 1e-9 {
                (b.deployed / total_deployed).min(1.0)
            } else {
                0.0
            };
            b.armor -= dmg * share;
    ...
    adjust_relation(
        ...
        &attacker,
        &old_owner,
        config.diplomacy.attack_delta,
    );
    ev(
        state,
        GameEvent::Siege {
            attacker: ship_id.to_string(),
            city: cid.to_string(),
            damage: dmg,
        },
    );
    if razed {
        adjust_relation(
            ...
            &attacker,
            &old_owner,
            config.diplomacy.capture_delta,
        );
...
pub fn colonize(
    ...
    let occupied: BTreeSet<String> = state
        ...
        .collect();
    let Some(settlement) = state.body(body).and_then(|b| {
        b.settlements
            .iter()
            .find(|s| !occupied.contains(&s.name))
            .cloned()
    }) else {
