...
.DESCRIPTION
  探针不是测试：它们要 GPU、要几分钟，所以不进默认的 `cargo test`
  （见 docs/verdicts.md）。**退出码才是判据**。
...
.EXAMPLE
  ...
  .\tools\px.ps1 -Task test                    # 快速链：默认 members（不含宿主与探针）
  .\tools\px.ps1 -Task test-all                # 全量：`cargo test --workspace`
#>
param(
    [ValidateSet(
    ...
    # ⚠ 参数名是 `-Task` 而不是 `-Target`：PowerShell 的参数名按**前缀**
...
try {
    ...
    # 只有 `-Task test`（`$extra` 为空、结果恰好一个元素）踩得到 —— 加了 `@()` 才真的是"一条命令"。
