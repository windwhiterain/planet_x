...
.DESCRIPTION
  ...
  **每一路都回一份结构化 JSON 报告**（`--report <路径>` 落盘，同一份也回给调用方）：
  ...
  ⚠⚠ **Perf 与 Stable 两路退休，别在脚本里把它们补回来。**
     它们要的是**计时用的帧循环**（逐帧采样 / 丢窗 / 等 K 帧 + 每条 pass 的编码器级
     GPU 时间戳），而今天唯一在的宿主（`px_render`）**按需渲染**：一条请求画一帧就回话，
     给不出逐帧序列、也没有那七段 span ⇒ 服务端收到这类请求**当场拒**。
     ⇒ 这两路由 `Stop-RetiredPhase` **当场拒并说清**（不是等超时、也不是等服务端
     回一句"这一路不在这一版"）。
     本宿主**有的**计时仪器是 `px_render --spans 预热,测量`：量**逐条 pass** 的编码器级
     时间戳 —— ⚠ 它**不是** `--perf` 的替代品，名字与口径都不同（那个数不叫 gpu_ms）。
  ...
  ⚠ 相位次序：`Invoke-StablePhase` 里 `Resolve-Artifact`（按 `target/pcg/<图>/manifest.json`
  解析路径）必须在 `Invoke-Bake` **之后**。反了不报错，而是**静默量错东西**：先取路径再重烘
  ⇒ 这一轮量的是**上一份**产物。⇒ 报告里那句"档"要能回答"我跑的是哪份产物"，否则数与产物
  对不上账。
...
-Phase $Phase 已**随锚退休**（S8-a）：这条请求这里不会发出去。
  ...
  **按需渲染**的：一条请求画一帧就回话，没有逐帧序列、也没有那七段 span。
  ⚠ 能给出可比 `gpu_ms` / `pair` 的那支宿主已经不在了，且不可重建（冻结的构建产物，
    重建出来的不是同一个字节序列 —— `docs/anchors.md`）。
