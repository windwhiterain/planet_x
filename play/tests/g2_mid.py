...
* **货舱（M2）**：`ships.cargo_capacity` = 舰级舱容 × 战损折算 `hull/hull_max`，且舰级舱容
...
def cargo_report(q) -> dict:
    ...
    `ships` 表现已发 `cargo`（按资源的在舱货物）与 `cargo_capacity`（有效舱容）。判据住在
    :func:`cargo_checks` 里。

    ⚠ `hull` / `hull_max` / `cargo_capacity` 都按 **r2** 发（读面约定：标量留两位压 token），
    所以公式对账走**舍入区间**（真值必须落在 `hull±0.005`、`hull_max±0.005` 推出的区间，
    ...
    for r in ships.to_dict("records"):
        ...
        if not (lo - 0.005 - 1e-9 <= got <= hi + 0.005 + 1e-9):
            formula_bad.append(f"r{r['round']} {r['ship_id']}（{cls}）：舱容 {got} 不在 "
                               f"[{lo:.4f}, {hi:.4f}]（hull {hull}/{hmax} × 舰级 {spec}）")
