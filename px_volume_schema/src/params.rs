...
pub mod emission {
    ...
    pub struct EmissionParams {
        ...
        ///
        /// ⚠⚠ 现在它**也乘 `main`、也乘星光散射那一笔**（`star_emit × star_lit`）：
        ///   含义升级为"**星云散射出来的光的通道配比**"（用户 2026-09-25）。星云不许自发光
        ///   （`emission_gain` / `glow_gain` 都关着）⇒ 画面上亮的那一片就是星光的散射，
        ///   而它必须是**红色** ⇒ 整条 `(main + glow + 散射)` 乘同一份配比。
        ///   `scatter_tint` 只负责"正式出图 / 分色诊断"的开关，不承担颜色本身。
        pub glow_tint: [f32; 3],
...
pub mod stars {
    ...
    pub struct StarsParams {
        ...
        /// **场星自己的颜色**（写进星表，逐星一份）。
        ///
        /// ⚠ 用户 2026-09-25："星光用蓝色、散射用红色。" 星表里的颜色只喂**直射**那一档
        ///   （`raymarch` 里 `power = 亮度 × tint`）⇒ 画面上的星走这个色。
        ///   ⚠⚠ "星光照亮气体"那一档（`bake_emission` 的 `star_lit`）**不读它**：那一笔的
        ///   语义是星光被气**散射**出来的光，逐通道加的是**同一个** `visible`（无色），
        ///   通道配比由 `EmissionParams::glow_tint`（红）给 ⇒ 红蓝分色才干净。
        pub star_tint: [f32; 3],
        ...
        /// **密度地板**（按“相对平均密度”给）：低于它的格子**一颗星都不留**。
        ///
        /// ⚠⚠ 用户 2026-09-25：“星云外面还是有很多星星，不行。”
        ///   纯幂律 `(d/平均)^对比度` 在气薄的格子上仍有可观存活率（比值 0.3、对比度 1.5
        ///   ⇒ 还有 ~16%）⇒ 空洞里照样撒满 ✗。地板把它**硬切到 0**：
        ///   `机会 = clamp((比值 − 地板)/(1 − 地板), 0, 1)^对比度`。
        ///   ⚠ `0` = 不切（从前那一档）；配合 `carved.out_min = 0` 用（真空洞里密度恰是 0 ✓）。
        pub gas_floor: f32,
        /// **按半径把星压向壳的外侧**（`0` = 关）。
        ///
        /// ⚠⚠ 用户 2026-09-25："让星星的分布更加在星云内部，看看拉高遮蔽能不能让星星减少到趋近于 0。"
        ///   相机在壳的**里面**（r = 0.05，气从 r = 1 起）⇒ **近侧**的星前面几乎没有气，
        ///   消光再高也遮不住它们 ✗（实测 `extinction = 150` 时云里仍有亮星 ✓）。
        ///   物理上能遮住的只有"视线穿过气"的那部分 ⇒ 把星的发生概率乘
        ///   `((r − inner)/(outer − inner))^gas_depth`，让它们集中在**外侧**：
        ///   越靠外的星，相机与它之间的气柱越长 ⇒ 透射率越低 ✓。
        pub gas_depth: f32,
    }
