...
/// 取"主光"：**就是场景里那盏点光源**（`clustered_lights.data[0]`）。
///
...
/// ⚠ **没有兜底**（用户口径，§64.9）：宇宙里没有平行光，第 0 格没被写过（uniform 数组默认值 = 全 0）
///    就是"这一帧没有光" —— 颜色 0、`point = 0`，画面全黑。多光源支持以后再改这里。
/// ⚠ `frag_coord` 只为不动四个调用点而留着（取灯不再需要片元坐标）。
