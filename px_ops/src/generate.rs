...
//!     `px_ops::field::Field`（`data` / `stats()`；取数口径逐字照抄，见 [`load_field`]）；
...
impl Palette {
    ...
    ///
    /// 渲染器那份返回的是 bevy 的 `LinearRgba`；`px_ops` 不认识 bevy，所以色调那一项
...
// ---------------------------------------------------------------------------
// 场的那两个小助手（`px_ops::field::Field` 上没有，照渲染器那份的数学原样搬）
// ---------------------------------------------------------------------------

/// `Field::normalized`：`px_ops` 把 `min` / `max` 放在 `stats()` 里，口径一样。
...
///
/// 需要先 `px_ops::begin(GraphSpec)`（它给出 `cache_root()`）；不方便依赖它时用
...
///
/// 需要先 `px_ops::begin(GraphSpec)`；不方便依赖它时用 [`write_generated_mesh_at`]。
...
/// 一份生成物的指纹（与 `px_ops::write_artifact` 用的是同一个函数）。审计/对账用。
