...
- **`docs/` 是文档的唯一根**：进门先读 `docs/current.md`（今天盘上成立什么），
  改任何东西之前读 `docs/invariants.md`（每条都付过代价）。
- **文件名与 `§` 号都是冻住的锚点**，理由与约定见 `docs/README.md` 的「命名与引用约定」。
  ⚠ 改文档文件名会连带换掉全仓缓存键 —— 因为 37 份**参与源码身份指纹**的 `.rs` 里逐字写着
  这些路径。先算清楚那个代价，再决定改不改。
- 文档必须与代码同步：过期的记录要么改、要么移进 `docs/archive/`，**不许留着当两份真相**。
- 讨论与想法记 `docs/rounds/`（按日期）或 `docs/backlog.md`（待办），不要只留在会话里。
