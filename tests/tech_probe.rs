...
fn probe_mond_control() {
    ...
    let checkpoints: Vec<u32> = [40, 120, 240, 400, 600, 800, 1000]
        ...
        .filter(|c| *c <= n)
        .collect();
    println!("== MOND 掌握度（前沿 0 → {:.1} AU；在场强度=Σ(1+深度×{})）==",
        sim::mond_frontier(&config, 0.0), config.mond.knowledge.depth_weight);
    for seed in seeds() {
        ...
        for name in &names {
            let c = sim::mond_control(&state, name);
            let presence_rate =
                *in_band_rounds.get(name).unwrap_or(&0) as f64 / n.max(1) as f64;
...
fn probe_customized_ships() {
    ...
    for seed in seeds() {
        ...
        for r in 1..=n {
            ...
            let fitted = state.ships.iter().filter(|s| s.hull > 0.0 && !s.components.is_empty()).count();
        ...
        println!(
            ...
            state.ships.iter().filter(|s| s.hull > 0.0 && !s.components.is_empty()).count(),
        );
    }
}
...
fn probe_mond_carrier() {
    ...
    for seed in seeds() {
        ...
        println!("== 承运抽成 seed {seed}（{n} 回合，有承运收入的回合 {rounds_with_carrier}/{n}）==");
        for f in &state.factions {
            println!(
