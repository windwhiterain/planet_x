...
> 这份是**入口**：下面每一条都是**今天盘上成立**的口径。历史与设计推演在 `docs/system/`
> 下那几份（`codegen-types.md` / `build-graph.md` / `generic-instances.md` / `operators.md`），
> 它们**保留原文**、读的时候先看过期标记。旧入口 `docs/archive/handoff.md` 描述的是本轮
> 之前的世界，**只当历史读**。
>
...
* **实例是「规格」不是「每个图一个」**：今天 7 条（`px list` 是真源）——
  `cloud.coarse/band`（体积域覆盖度）、`field.remap/waves` 与 `field.remap/latbands`（同一份规格、
  两个体文件 ⇒ **同一把 key**）、以及 element 那一档的 `field.constant` / `field.mix` /
  `field.remap` / `field.fuse`（同一份泛型规格、四个体文件 ⇒ 同一把 key）。
...
px_render --view --scene <S.pxart> [--edit <场景配方名>]
                                         预览窗口 + **调参面板**（S9，见 docs/render/viewer-panel.md）
...
⚠ **anchor 与实例 key 都是"按 checkout"的量**：行尾（CRLF/LF）不同、甚至只改一行注释，
同一个 recipe 就会给出不同的值（注释也进源码指纹）。**换一份 checkout 要重跑一遍登记。**

⚠ **anchor 自 2026-09-20 起不再重登记**（`.pxart` 六格）：锚宿主已删，登记无法再验证 ⇒
此后判据 = **内容逐字节**（节点产物内容哈希 / 判据图像素）＋ **同机位出图 + 所改那一栏的读数**。

⚠ **盘上有一处陈旧**（2026-09-28 实测，**按裁决只记录不改**）：`art/nebula/density_volume.toml`
还是旧字段 `res_ratio`，而 `DensityParams` 只认 `res` ⇒ **`px run nebula` 在今天烘不过**
（`docs/system/params.md` §2 那次改名漏了这份 toml）。见 `docs/render/viewer-panel.md` §6。
...
5. `px list` → `px build` → `px run <图> --build`。
   ...
   参数文件 / 生成物留着"四行映射（见 `docs/system/codegen-types.md` §7 与指南 §5.5④）。

## 六、记录地图

| 文件 | 是什么 |
|---|---|
| `docs/render/viewer-panel.md` | **最近一轮**：预览窗口里的调参面板（改参数 → 子进程 cook → 画面重载） |
| `docs/system/codegen-types.md` | stage 1 生成类型（recipe + `px_decls` + `insts_gen.rs`）+ 读数 |
| `docs/system/build-graph.md` | build graph / 两阶段 / `px` driver / §192–§195 的清理与裁决 |
| `docs/system/generic-instances.md` | 泛型实例（`px_inst!` 时代）—— key 各轴 / 握手 / 装载仍然生效 |
| `docs/system/operators.md` | 实现库拆分与 M1/M2；那些用血换的教训仍然有效 |
| `docs/guides/writing-an-operator-library.md` | 面向使用者的指南（§5.3–§5.5 是本轮的形状） |
