...
> 本篇的 §159 是**当时那一轮**的裁决；今天生效的形状以 `docs/system/operators.md` 为准。
...
>
> § 号接 `docs/archive/render-wgpu.md`（§157/§158）。这一篇是**决议 + 事实 + 判据**，读数随做随记。
...
> 替代门与逐字节判据（14/14）见 **`docs/system/typed-scripts.md` §162/§163**。
| S5 | 文档指针与旧裁决标注 | **部分**：`docs/system/pcg.md` §15.2 那条「算子静态链、cdylib 留接口不做」**已被这一轮取代**（那一条的理由是「只买到不重启图程序」，没算上「改算子不必重编图程序」与「场函数过不去边界」这两条）；本文件即取代记录 |
...
> **`px_inst!` + build graph** 的形式回来了（`docs/system/generic-instances.md` §174–§180、
