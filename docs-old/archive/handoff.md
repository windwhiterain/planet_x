...
> ⚠ **S8-c 标注（2026-09-19）：这一篇不再是"唯一的现状出口"，而是一份日期化的记录**
> （2026-09-15/16 那几轮）。现状出口从 `art/docs/archive/render-wgpu.md` 起就是**那一篇**（§100–§156，
> 整个 S 系列的过程与判据都在那里）。**正文与全部读数一字未删** —— 它们是那几轮唯一的记录。
> ⚠ 三条会读错的地方，先看这里：
> 1. 正文里几十处 `px_render`（Bevy 宿主）**已经删了**（`f121ee3`，`docs/archive/render-wgpu.md` §154）
>    ⇒ 那几条**命令今天跑不了**：§9.6 的 `-Target test-all`（今天照跑，只是 `--workspace` 里
>    没有 `px_render` 了）、§9.7 文件地图里 `px_render/src/*` 与 `px_render/assets/shaders/*`
>    两条落点（shader 库搬到了 `art/shaders/lib/`，§155）。
> 2. 那些 `px_render/src/…` 引用**不是死引用**：源码在 git 历史里
>    （`git show f121ee3^:px_render/src/main.rs`）。⚠ 与 `bevy_pbr-0.19.1` 那一类不同 ——
>    后者在 cargo registry 里，今天仍可直接打开（`docs/archive/render-wgpu.md` §156.2）。
...
>    `docs/system/graphs.md` 与 `docs/system/operators.md`；今天怎么写算子读
>    `docs/guides/writing-an-operator-library.md`。
> 5. ⚠ **参数名已改：`-Target` → `-Task`**（照抄本文里的命令会报"参数不存在"）。
>    今天见 `docs/system/build-graph.md` §191。
...
**还没并入 `v2`**。到这一轮为止**只有笔记**：`docs/render/graph-research.md` §67–§78（动态 schema 与动态 render graph 的调研、
四条用户裁决、修订后的路线与开工顺序、裸 wgpu 与 Bevy 图能有多动态的追问），本轮的实测**单独一篇**：
**`art/docs/render/schema-limits.md` §79**。
...
**做了什么**（读数与四道墙在 `art/docs/render/schema-limits.md` **§79**）：借 `.worktrees/generic-render/target` 当构建缓存
...
a4 **4.633** / b12 **4.537 ms**（−2.1%，噪声内）｜84 个用例通过。细节在 **`art/docs/render/renderer.md` §80**。
...
也在编辑 `docs/render/graph-research.md` —— 本轮的实测因此**另立** `art/docs/render/schema-limits.md`（用户裁决）；
`docs/render/graph-research.md` 第 734 行那份副本**已删**（2026-09-16，原地留一行指路）。
...
细节在 **`art/docs/render/renderer.md` §81**。
...
**下一步的位置**：`art/docs/render/graph-research.md` §76 开工序的第 3 步（**pass 图**：文档里的 pass 表 + 一个执行器系统）
—— 那条线在 `.worktrees/pass-table` 里由另一个会话并行做，动手前先跟那边对齐。

#### 再续（同一天）：尾巴三件已提交 **`7f8e144`**（另加探针修复 **`9c0add2`**）

细节在 **`art/docs/render/renderer.md` §82**：
...
3. **笔记清理**：`docs/render/graph-research.md` 里那份 §79 副本已删（原地留 5 行指路），三处引用同步。
...
⚠ **合并把两个分支的 § 号撞在一起了**：`§79` 被 `docs/render/schema-limits.md` 与 `docs/render/pass-table.md` 同时用，
`§80 §81 §82` 被 `docs/render/renderer.md` 与 `docs/render/pass-table.md` 同时用。
按「后合进来的那条让号」把 pass 表那篇整体挪到 **§85–§90**（本次调研跟着挪到 §91–§99；
**§83 §84 空着是这次挪号留下的断档**）。`docs/invariants.md` 的篇表与 § 号表已同步，代码注释里的
...
细节与判据在 **`art/docs/render/pass-table.md` §86.4**。
...
**剥 bevy 的调研**（**代码未动**）另立一篇：**`art/docs/archive/bevy-exit.md` §91–§99**。
...
产物 156.6 → 8.45 MB、进程到设备 1.31 → 0.45 s。

**用户裁决：跳过阶段 0，直接做阶段 1。** 开工序是 **`art/docs/archive/render-wgpu.md` §100–§107** ——
那是**给新 session 的工单**（边界 / 判据 J1–J6 / 已经搭好的件 / 要新写的东西 / 十三个坑 /
S-1→S8 的阶梯，每档一判据）。要点三条：

1. **主判据是逐字节**：四档场景 + `--sheet` 12 格与 bevy 宿主逐字节相同。
   不逐字节相同的剥离 = 把 §51 那一整套性能账清零 ⇒ **`px_render` 是锚，判据全绿之前一步都不能少**。
2. **唯一允许动的共享件**是把 `px_shader::assemble` 的桩表改成宿主可提供（§103.1）——
   两个宿主对同一批 `#import bevy_pbr::*` 的兑现方式不同（Bevy 走 naga_oil，裸 wgpu 只能自己兑）。
   内容 shader 在这一步**一个字都不改**；改名成 `planet_x::*` 是最后一步（S8）。
3. **第一道真判据是 S3 的 `orbit-bare` 逐字节**（行星 + 大气 + 一盏点光 + 星空盒）；
   点光 cube shadow map 是全局唯一没有现成参考的活（§104 第 2 条）。
...
- 端到端闸门实测（Vulkan / RTX 3060 / 960×640 / 一个服务会话）：基线出图 300012 字节 → 给
  ...
  再重烘 ⇒ 键**逐字节回到基线**（键是纯函数）。细节在 `docs/render/renderer.md` §52.3。
...
5. **热重载那条残留**（`docs/render/renderer.md` §52.3 写着）：服务在跑时改库文件，watcher 会把新模块组装进去、
...
8. **材质参数 schema 的调研（§66）**：只落笔记，**代码未动**；候选批次 P1 / P2 / P3 见 `docs/render/renderer.md` §66.5。
...
**这一轮做了什么**（用户的两条要求，口径与实测全在 `docs/art/clouds.md` §59）：
...
- 差异带（`target/pixdiff.ps1`，逐像素最大通道差）：只换材质 max 47、云影 max 84（13.4% 像素）、
  shadow map max 229 但只有 0.68% 像素 —— 数字与图都在 `docs/art/clouds.md` §59.3。
...
口径、实测与踩到的坑全在 `docs/art/clouds.md` **§60** 与 **§61**。
...
- **② 已做**：口径、落点表与实测全在 `docs/render/renderer.md` **§62**。要点：`pipeline_gate` 成了出图前
...
与 **"确保平行光被删除了，从渲染器里"**。全过程与证据链在 `docs/art/clouds.md` **§64 / §64.9**。
...
`docs/render/renderer.md` **§65**（这一轮新增），这里只写"做到哪了"。
...
6. `docs/` 里其它篇（`system/pcg.md` / `system/assets.md` / `art/clouds.md` / `art/gradient.md` /
   `render/instruments.md`）仍按旧形状描述 KINDS/槽/`spawn_planet` ——
   这一轮**只**新增 §65，没有逐篇回改。
...
- **环**（唯一一条从没跑过的路径）：新增配方 `art/scene/orbit-rings.toml` 并出图，
  ...
  `target/rings-shot.png`（1200×800）。细节见 `docs/render/renderer.md` §65.1。
...
> ⚠ **本节的命令行是旧接口（原文保留）**：内容旗标已在 §52 / P10 删除，内容只走 `--scene`；用法见 `docs/render/instruments.md` §40 与 `docs/render/renderer.md` §52。下面的 `--planet / --mesh / --clouds / …` 只能当历史形状看。
...
> **P11 状态**：viewer 的契约与坑已由 `docs/render/instruments.md` §56 / §56.1 补齐（含"起窗口必须脱离"、判档日志、修复没进 exe 的两个坑）⇒ 本节状态以 §56 为准；原文保留。

**P14｜装过 shader 之后要等 READY**：现在只等"管线入队被看见"（有界 40 帧，超时带警告照常出图）。
如果一份 WGSL 真换了内容，那一张仍可能赌输。要彻底就得等 READY，但要处理"管线永远编不出来"时别把任务挂住。

> **P14 状态**：⚠ v11 起主路径改成"等条件"（管线全部就绪 + 8 渲染帧，见 `docs/render/instruments.md` §57/§58）⇒ 本节"只等入队（有界 40 帧）"是旧形状；原文保留。
...
> **P18 状态**：第 3 条（显存泄漏）**已闭环** —— 修法与验证在 §51.12，根因与上游 issue 文字在 `docs/render/instruments.md` §55；第 1 条（缠绕）仍按渲染侧自动翻面处理，第 2 条（逐字节判据）仍未达到（§51.11）；原文保留。
...
> **P17 状态**：⚠ v11 起判据侧由 `has_cloud` 兜（`docs/render/instruments.md` §57/§58.1）、"热身 4 s"已退休（§51.18）；本节的手工步骤属旧协议/回退路（`--windows`/`--drop`）的形状；原文保留。
...
| 想知道什么 | 看哪个文件 |
|---|---|
| 路线裁决与「为什么不那样做」 | `art/docs/decisions.md` |
| PCG 图程序怎么写、缓存键与版本号 | `art/docs/system/pcg.md` |
| 产物里有什么（域 / 指纹 / 相机表 / diff） | `art/docs/system/assets.md` |
| 球面网格、位移、法线、mip、接缝与极点 | `art/docs/art/geometry.md` |
| 天空与大气（含 Bevy 散射大气的挂起状态） | `art/docs/art/sky.md` |
| 体积云与覆盖度 | `art/docs/art/clouds.md` |
| 云密度场的梯度：组装形式与判据 | `art/docs/art/gradient.md` |
| 渲染器怎么跑（离屏 / 服务 / viewer）＋ 资源缓存 | `art/docs/render/renderer.md` |
| 改完怎么验（热重载 / review 回路 / 单帧时间 / 探针） | `art/docs/render/instruments.md` |
| 不变式（每条都付过代价） | `docs/invariants.md` 顶部 |
...
分配点行号、三选修法实测）在 `docs/render/instruments.md` §55.4，**没有真去提交**。
