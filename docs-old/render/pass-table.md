...
> 起因：`docs/render/graph-research.md` §73 的四条裁决（不重编 / 不重启 / 不要编辑器 / 要 compute）与 §74 的四步路线。
...
> ⚠ **S8-c 标注（2026-09-19）：这一篇的"形状"仍然有效，只有 `px_render` 那几条**落点**过时了。**
> **正文与全部读数一字未删。**
> 1. **仍然承重、一个字都不许动**：§85 的三层划分、§86.1 那四条判据（A/P/R/D）的**读数**、
>    §86.4 那条裁决（pass shader 与材质**共用一份契约**）、§87 那两个真 bug 的根因与修法、
>    §88 的明账。§146/§147 在 `px_render_wgpu` 上重跑过同一批判据（J3），数一致。
> 2. 过时的是**落点**：`px_render::passes::executor_layout()` / `px_render::passes::resolve` /
>    `px_render/src/{passes,main}.rs` 这些路径的宿主**已删**（`f121ee3`，`docs/archive/render-wgpu.md` §154）；
>    今天同一份逻辑在 `px_render_wgpu`（§146.7 有落点表）。§89 那张文件清单是**当时**的清单。
>    ⚠ 引用可查：`git show f121ee3^:px_render/src/passes.rs`。
> 3. §86.1 的 **P｜不重编** 那一格量的是 `px_render.exe` 的 sha256（`2C8858917C63B704…`）——
>    **那是记录**，不是今天能重量的一格；它的**结论**（改 shader 内容不需要重编）由 §152 的
>    热重载判据在 `px_render_wgpu` 上重新立过（272 ms 内画面变）。
>
> ⚠ **§157（2026-09-19）：上面两处 `px_render_wgpu` 今天叫 `px_render`**（wgpu 宿主改名，
> `docs/archive/render-wgpu.md` §157）。⚠ 而这一篇里的 `px_render`（第 ①/②/③ 条那些落点）**照旧指
> 已删的 Bevy 宿主** ⇒ 同一段里两个名字**各指一支**，按日期切。
...
**撞到了什么**：v2 的「槽表扫目录」（`docs/render/renderer.md` §81.2）把 `art/shaders/*.wgsl` 里**每一份**入口
...
> ⚠ **就地更正（`docs/archive/render-wgpu.md` §146.5 实测，2026-09-16）**：上表"旧"那一栏的
...
**一条被这次实测改掉的旧话**：`docs/invariants.md` 原来写着「pass 的绑定布局不像材质那样固定」——
...
| `px_graphs/src/bin/shaders.rs` | **不用改了**：槽表从写死的数组改成扫 `art/shaders/*.wgsl`（v2，`docs/render/renderer.md` §81.2）⇒ 两份 pass shader 自动进表 |
