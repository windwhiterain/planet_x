...
以及它怎么记住自己造过的东西。**改完怎么验**在 `docs/render/instruments.md`。
...
> Bevy 宿主**（`f121ee3`，`docs/archive/render-wgpu.md` §154）⇒ 今天用 `px_render_wgpu --offline --scene …`；
...
> 同一段两个名字各指一支，按日期切（`docs/archive/render-wgpu.md` §157）。
...
见 `docs/archive/handoff.md` §9.1.7 第 3 条"云自己的曝光没动"）。
...
> 于是又调研了一轮，落成 **`art/docs/render/graph-research.md` §67–§73**。三条与本节直接相关的更新：
...
3. `--view` / `--sheet` 没跑。
4. `art/docs/render/graph-research.md` 里那份重复的 §75 副本**已删**（2026-09-16，原地留一行指路 → 本篇 §79）。
...
`art/docs/render/graph-research.md` 里那份 §79 的副本（节号撞成 §75、与那边的「裸 wgpu」同名）**已删**，
原地留 5 行指路（`docs/render/schema-limits.md` §79）。引用了它的三处（`docs/render/renderer.md` §81.5、
`docs/render/schema-limits.md` 末条、`docs/archive/handoff.md`）也一并改成「已删」。
