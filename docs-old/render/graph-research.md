...
> 代码未动。§ 号接 `docs/render/renderer.md`（§66 是同一件事的前一次调研：只到「参数块」，没到「render graph」）。
...
| 文档不认识的 kind 已经不存在了（`KINDS` / `assembler` / `PlanetSpec` 全删） | `docs/render/renderer.md` §65 的搬走清单 |

**⚠ 一条过期笔记**：`docs/render/instruments.md` §42.1 那张表写着「改 `CloudParams` 之类的 uniform 结构 ⇒ 必须重启」。
...
**改 WGSL 结构体 → 重烘 `shaders` + `scene` → 出图，全程不重编 exe**；再把 `docs/render/instruments.md` §42.1 那张表改掉。
...
- **⚠ 这是一次明确的路线反转**：§16.6 把「图不再是可读数据 / 美术自己拖节点不在路线图上」写成了**裁决**，
  ...
  才需要真正的图引擎**」（`docs/decisions.md:93`、`docs/system/pcg.md:10`）。
...
3. **改** `.wgsl` **文件就生效**（开发期那 1 秒的近路）：今天只在「槽里的版本」这一层通了；
   ...
   §52.3 与 `docs/archive/handoff.md` P12 记的正是这条：引擎那半截已经在了，缺的是工具层的 watcher。
...
2. **A3（图 + 编辑器）是本方案里唯一一个「新架构」**，也是唯一一次**明确的路线反转**
   （§16.6：「图不再是可读数据 / 美术自己拖节点不在路线图上」；`docs/decisions.md:93` 原话是
...
### §74.4 材质的硬顶：**裁决 = 先 (a) 一次性加宽超集**（2026-09-16）

裁决说「render 不需要重新编译」，而**只要 schema 还在固定超集里，就有顶**：
4 张贴图、参数块 1024 字节（`reflect.rs:25-32`）。超了就必须改 Rust ⇒ 撞判据 P。

**用户裁决：先做 (a)** —— 一次性把超集加宽（`参数块 + N 张 2D + M 张 cube`，参数上限提高），
**(b) 自建材质绘制路径作为后备**，等美术真的撞到顶再动。

| 选项 | 做法 | 代价 | 结论 |
|---|---|---|---|
| **(a) 加宽超集** ✅ 已选 | 一次性把超集改成「参数块 + N 张 2D + M 张 cube」（建议 8 + 4）、参数上限提到 4 KB | 一次性 Rust 改动；每条管线多几个空格（§65 实测「多绑 6 个空格无代价」） | **做**。判据 P 从此是「顶以下恒成立」，而顶高到实际用不到 |
| **(b) 自建材质绘制路径**（后备） | 不用 `MaterialPlugin`：自己建布局、自己建管线、自己走一个 draw 阶段（`FullscreenMaterial` / `bevy_anti_alias` 是现成模板） | **大**：prepass / 阴影 / 变体要把 Bevy 那条路接管过来 | **暂不做**；触发条件是「美术真的撞到顶」 |

⚠ **加宽超集不是纯加字段**：它同时动三处 —— 布局（`material.rs:98-133`）、
占位 WGSL 手抄的那张表（`slots.rs:167-187`）、以及 §66.2 那五处手抄里的其余几处。
⇒ **它应当和第 1 步（契约收口）一起做**：先把表收成一份，再在那一份上加宽，只改一个地方。
顺序反过来就要改五处，而那正是 §66.2 记的那个病。
...
0. ✅ **已做（2026-09-16）**：**改 WGSL 结构体（加一个参数）→ 重烘 `shaders` + `scene` → 出图，
   全程不重编 exe、不重启服务** —— 八步读数与逐条墙在 **§75**（`docs/render/instruments.md` §42.1 那张表已按它改掉）。
...
2. **第 1 步（schema 数据化）**：配方透传 + `SLOTS` 扫描 + 协议加法不升版本（含拒未知字段）。
...
## （已迁出）第 0 步实测 → `art/docs/render/schema-limits.md` §79

> 这一节原来写在这里，但 §67.4 / §69 那几条的**实测正本**已经另立一篇：**`art/docs/render/schema-limits.md` §79**
> （八步读数、判据 P/R/D、四道墙 W1–W4、两个硬顶的实测值、复跑命令都在那儿）。
> 留这一行只为让从 §74/§76 跳过来的人找得到路 —— 别在这儿续写。
