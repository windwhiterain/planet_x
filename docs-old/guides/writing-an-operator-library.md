...
> 机制与读数记在 `docs/system/operators.md`（为什么运行期装载、握手、
> 身份两半、搜索路径）；这一篇只讲**怎么写**。
...
> `g.inst::<T>()`"的写法**不要再照抄**。今天见 `docs/system/codegen-types.md`
> （recipe + 声明表 + 生成器），以及 `docs/system/generic-instances.md` 顶部那条过期标记。
> 下面这段旧示例只作历史：
>
> ```rust
> // ⚠ 旧写法（已废弃）—— 图侧一行宏 + 一行登记
...
```bash
# ⚠ 只有一个 driver `px`（`px_jit` / `px_run` 本轮并成它，见 `docs/system/build-graph.md` §191）。
...
（`px_graph::driver` 的模块文档有整段；界面上的用户是调参面板，`docs/render/viewer-panel.md`）。
...
详见 §5.5 与 `docs/system/codegen-types.md`。
...
（`docs/system/elementwise.md`）—— 两者的**人读名相同**是有意的（"值域重映射"这件事只有一个名字），
...
设计全文与全部读数在 `docs/system/codegen-types.md`；这一节只讲**为什么是这个形状**。
...
包括一行注释，都会换掉它的 `SOURCE_HASH`，于是全部实例 key 与节点键一起换、既有产物键全部作废。）
...
10. **实例那张数据收据（recipe）有几条硬口径**（`docs/system/codegen-types.md`，都是实测踩出来的）：
   ...
   * ⚠ 声明事实**不许**搬进 `px_*_schema`：那会给 schema 加字节 ⇒ 换 `SOURCE_HASH` ⇒
     全部实例 key 与节点键一起换、既有产物键全部作废。`px_decls` 不进任何实现库名册 ⇒ 不换键。
