...
配套的**探针**（`px_probe`）与**参考图**（`art/reference/`）不是附属品 —— 它们是"这个工程
还能不能被验证"的载体。（曾经的**锚** `art/anchor/` 已于 2026-09-28 退休：
它是与**已删的** Bevy 宿主逐字节对齐的登记，那个用途随宿主一起消失。
为什么退休、以及从它身上还剩哪些仍然生效的禁令，见 [`anchors.md`](anchors.md)。）
