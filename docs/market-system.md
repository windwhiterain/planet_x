...
5. **读数仍会走到极端**：同一配置（`modern --capacity 12 --specialty 2`），5 个随机种子
   20000 轮落在 `1e-8…1e6`，但 **`seed 11` 只用 2000 轮就把两个商品读数打到 `0`、第三个
   打到 `2.4e8`**。所以"有界"只是多数轨迹的性质，**不是保证**。`index = 0` 说明
   "某一轮一本账都没成形"这条路径还没处理干净（`Lab::step` 只在 `price > 0` 时覆盖，
   `anchor_prices` 对 0 无能为力），下一步要查的就是它。
...
实测：5 个随机种子 × 20000 轮，`log10` 三商品 index 的 max/min = **2.9–8.4 / −8.2 – −4.5**；
但 `seed 11` × 2000 轮就会出现 **0 / 2.4e8**（见 §7.5）——**"有界"不是保证，只是多数轨迹**。
