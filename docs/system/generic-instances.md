...
> **`docs/system/codegen-types.md`**；宏本体也已作为死代码删除（`px_cook/src/lib.rs` 只剩一行注记）。
...
> § 号接 `docs/system/operators.md`（M1：声明住 schema、实现住 dylib、按身份装载）。
...
—— 见 `docs/system/build-graph.md` §182/§186）。⚠ 当时另有一个 `PX_JIT=build` 环境变量兜底，**后来废弃并已从代码里删除**。

## §175 形状（图侧只写一行）

> ⚠ 标题里的"只写一行"**已过期**（2026-09-20）：图侧现在是**一行数据**（`inst_recipe.rs` 里的一条
> `InstRecipe`），类型由 stage 1 生成 —— **既没有宏、也不手写类型**（`docs/system/codegen-types.md`）。
> 下面这一行是**这一轮的形状**——第 4 段当时是**单个** alg crate 字面量。
> 它今天成了**根列表** `["px_volume_alg"]`（可多个，= 这个实例编译时链了谁，`docs/system/build-graph.md` §183，
> 今天住在 recipe 的 `roots` 那一栏）。以 `19` §179.1 与 `docs/system/codegen-types.md` 为准。
...
见 `docs/system/build-graph.md` §191.2）。key 那一行里的 "alg 源码名册" 今天是**根列表里每个 crate** 的名册。
...
> （`docs/system/build-graph.md` §183/§188）。理由与本节一模一样，只是从"一个"变成"一组"。
...
> `OUT_DIR/insts_gen.rs`、`insts.rs` `include!` 进来 —— 见 **`docs/system/codegen-types.md`**。
...
> ⚠ 就地更正（2026-09-20，`docs/system/build-graph.md` §183/§189/§190）：第 4 栏是**根列表**
> （`INST_ROOTS`，`["px_volume_alg"]`，可多个）—— 它今天住在 **recipe 的 `roots` 那一栏**，
> 而 `build()` 改成遍历 `recipe::INSTANCES` 调 `g.facts(…)`（`docs/system/codegen-types.md`）。
...
> ⚠ 已过期（2026-09-20，`docs/system/build-graph.md` §185/§190/§191）：`px_jit` 已退化成**薄 driver**，
...
> **也没有了** —— 模板原文今天住在 recipe 的 `body` 那一栏（`docs/system/codegen-types.md`），
...
> —— 见 **`docs/system/build-graph.md` §184**（单态化 interface：声明处只有 `build()`）、
...
> `list`/`build`/`gc`/`run`、参数名 `-Target`→`-Task`；见 `docs/system/build-graph.md` §191.2）；`inst_scan` 也只剩
