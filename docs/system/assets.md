...
（PCG 侧的缓存键与版本号在 `docs/system/pcg.md`。）
...
⚠️ **频率语义变了**：单位球绕赤道一圈是 2π，`frequency = f` ⇒ 赤道上约 `2πf` 个特征，旧参数（3.2 / 11.0）按 3D 语义相当于「赤道上 20 个格子」，必须重调；现为大陆 `0.55`、山脉 `1.9`、沙漠台地 `0.45`、峡谷 `3.2`。`field.warp` 仍是**图空间**扭曲（在像素坐标里偏移采样、极点附近并不球面正确），后由方向空间的矢量位移取代（`docs/art/geometry.md` §33）。
...
> 也一并删掉了。见 `docs/guides/writing-an-operator-library.md` §3.7。
...
单个噪声节点烘焙 18–29 ms → **287–297 ms**；planet 图全量 67 → **619 ms**；desert 图全量 96 → **779 ms**；出图 0.34 → 0.37 s；**整轮热路径 0.49 → ≈ 1.0 s**。若嫌慢，可把画布降到 512×256（约省 55%）。mip 见 `docs/art/geometry.md` §26。
...
### §28.2 源码哈希有个洞（**已修**，2026-09）

算子原先只哈希**自己那个文件**（`include_str!("fbm.rs")`）⇒ 改**共享代码**（`field.rs` 里的方向约定、`noise.rs`）时，§19.1 那条「源码变了」警告**不会响**（当时手动把 `field.fbm` / `field.ridged` 升到 **v4** 顶过去）。

现在的口径：`SOURCE_HASH = fnv1a_sources(&[…])`（`px_ops::noise`，`const fn`，每段带长度前缀，`["ab","c"]` 与 `["a","bc"]` 不许撞）：

- `px_ops/src/ops/*.rs` 八个算子：自己的文件 + `../field.rs` + `../noise.rs`（**统一一份列表**，不逐算子维护：多哈希一个文件最多多响一次告警，少哈希一个就是静默陈旧命中）。
- `px_mc/src/lib.rs`（等值面算子）：自己的文件 + `px_ops/src/volume.rs`（体积与面参数的约定）。
- `px_graphs/src/cloud_proxy.rs`：自己的文件 + `px_ops/{field,volume}.rs` + `px_verify/{cloud_field,noise,dual}.rs`（它复算的就是那份参考场）。
...
> ⚠ **§159 取代（2026-09-19，`docs/system/graphs.md`）：上面那三条路径全部搬了家**，口径不变、读数一字不删：
...
- **材质的参数 schema 走的是同一本账**：`Material.params` 那袋值（`px_protocol::scene::Value`）只是**载体**，参数名 / 类型 / 字节偏移是渲染器从产物里那份 WGSL 反射出来的 ⇒ 现状、硬边界与「要不要挪进 protocol」的五条候选形态见 `docs/render/renderer.md` **§66**。
