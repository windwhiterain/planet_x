...
>
> ⚠ 参数名已改：`-Target` → `-Task`（照抄本文里的命令会报"参数不存在"）。今天见 `docs/system/build-graph.md` §191。

> **这一篇是工单，不是调研。** 调研与全部实测读数在 `art/docs/archive/bevy-exit.md` §91–§99。
...
> ⚠ **这句"今天的 `px_render`"已被 §154 取代**（S8-c 标注，原句一字未删）：目标两半都兑现了 ——
> 逐字节同图（§145/§146，J1/J2/J3 全绿），然后**取代**（`px_render` 已删，提交 `f121ee3`）。
> ⇒ 这一句现在读作"**当时**的目标"，而不是"仓库里还有两支宿主"。
...
- 不许删 `px_render`（Bevy 宿主）—— 它是**判据的锚**，判据全绿之前一步都不能少。
  > ⚠ **已被 §154 取代**（S8-c 标注，原句与读数一字未删）：`f121ee3` 已经把 `px_render` 删了 ——
  > 因为这条禁令的前提（"判据全绿之前"）在 §145–§149 走完了，而 §154 之后判据**不再需要第二个宿主**：
  > 靶子（六张判据图 + 六份冻产物）已经搬进仓库 `art/anchor/`。
  > ⚠ 反面要一起读：**锚 exe 和它"提问"的能力一起没了**（`art/anchor/hashes.txt` §五、本篇 §143），
  > 所以"锚"这个词从 §154 起指的是**仓库里那六张图与六份冻产物**，不再是一支可执行的 exe。
...
| **J3** | pass 表四条判据全过 | `art/docs/render/pass-table.md` §86.1 的 A/P/R/D（六张图的哈希见 §86.4） |
...
> ⚠ **J6 的第二半已被 §154 取代**（S8-c 标注，原句一字未删）：`cargo test -p px_render`
> 今天**跑不了** —— 那条命令的原文报错留在 §154「"px_render 那一行"是**没有了**，不是变绿了」。
> 它的**意图**换个形式活着：**"不能改坏既有门"** 现在是九个 crate 的测试 + J1/J2/J3 +
> 逃生门六份（§154 判据）。⇒ 读这一格时把"Bevy 宿主"读成"既有门"，其余照旧。
...
> ⚠ **这一节里那些 `px_render::…` 落点，今天都搬到了别处 —— 已被 §154 取代**（S8-c 标注，
> 表格一字未删）：CAS 装载 / 网格 / 材质 / 反射 / 桩表分别住在 `px_render_wgpu::art`、`::mesh`、
> `::material`、`px_shader::reflect`、`px_shader::host_stubs`。⚠ 而"**不要重写**"这条纪律本身
> 照旧：它们都已经搬完并各自带判据，今天更没有理由重写。
...
> ⚠ **"两个宿主必须同时活着"已被 §154 取代**（S8-c 标注，原句与两条理由一字未删）：
> Bevy 宿主已删 ⇒ 这句今天没有对象。⚠ 但**它当时解出的那条设计仍然在代码里**：桩表作参数、
> 组装器只有一份、两张表并存（`px_shader::host_stubs` + `assemble::bevy_stub`），只是消费方
> 从"两个宿主"变成"烘图侧与唯一宿主"。
> ⚠ 次序值得读一遍：§125 先把锚**降级**成"冻结的 oracle（不再演进）"，§154 才把它删掉 ——
> 不是一步跳过去的。真正的代价记在 `art/anchor/hashes.txt` §五。
...
> ⚠ **这条证明今天做不了 —— 已被 §154 取代**（S8-c 标注，原句一字未删）：没有 Bevy 宿主可重编、
> 可重跑。它的**等价物**是 §154 的那条判据：重烘 → 重出 → 与 `art/anchor/*.png` **逐字节相同**
> 且逃生门六份**文件字节**相同。⚠ 原句保留是有用的：它是 S1 那一步**当时真正跑过的判据**，
> 读数在 §108.3 ②（五格全等，"预期不算判据，哈希算"那句就出在那儿）。
...
> ⚠ **"改名是最后一步"已被 §155 取代（裁决 a：一个符号都不改）**（S8-c 标注，原句一字未删）：
> 那一档**量过之后决定不做** —— 改名会换掉**所有产物键**与六份冻文档的**文件字节**（§155.2/§155.3
> 的表），而**像素一个都不动**。⇒ 那不是"改语义"，是"换靶子"，只能由用户/评审裁。
> ⚠ 前半句"内容 shader 一个字都不改"**仍然有效，而且更硬**：连注释都不许改（§155.4 第 1 条）。
> ⚠ 还有一层今天才量清：`art/shaders/*.wgsl` 与产物键绑的不只是**内容**，还有盘上那些字节的
> **行尾约定**（`art/anchor/hashes.txt` §六）—— 换一台 checkout 约定不同的机器重烘，键会变。
...
> ⚠ **这一节是 2026-09-16 的工单，已全程走完**（§108 → §154）；S8 那一档的形状与这里写的有出入
> （逐条就地标注在下面）。⚠ 其中 S1/S2/S3/S6 那几格判据的**形式**是"两个宿主各出一张比哈希"，
> 第二个宿主已删（§154）⇒ 今天那份判据的等价形式是"**与 `art/anchor/*.png` 逐字节相同**"（J1）。
> ⚠ 原文与读数一字未删：这份工单是"当时按什么做的"的唯一记录。
...
**`63184151909371A5`**（`art/docs/render/pass-table.md` §86.1 的既有读数，§86.4 那张对照表也是同一个数）。
...
2. 把 `art/shaders/*.wgsl` 里的 `#import bevy_pbr::*` **改名**成 `#import planet_x::*`
   ...
   > ⚠ **已被 §155 取代（裁决 a：一个符号都不改）**（S8-c 标注，原句一字未删）：这一条量过之后
   > **决定不做** —— 那些名字是**出处指针**，不是过时的名字；改名会换掉所有产物键与六份冻文档的
   > 文件字节，而像素一个不动（§155.2/§155.3）。
3. 重烘 → 重出 → **J1/J2/J3 必须还是逐字节相同**。这一条过了，才叫"剥离完成"。
   > ⚠ 这一条**照旧有效且已兑现**（§154 判据，S8-c 又复跑一遍）：只是"逐字节相同"的对手方从
   > "Bevy 宿主出的图"变成 `art/anchor/*.png`（靶子已进仓库）。
4. 删 `px_render`（Bevy 宿主）、删 `px_shader::assemble` 里的 bevy 桩表、删 `slots.rs`。
   > ⚠ **只兑现了一半**（S8-c 标注，原句一字未删）：`px_render` 与 `slots.rs` 随那个 crate 一起删了，
   > 而**那张桩表留了下来**（`px_shader::assemble::bevy_stub`）—— §155 量出它今天还有真实用户：
   > `px_ops` 的烘图路、`px_graphs::frame` 烘帧材质，以及 `px_render_wgpu::stubs` 那条**对照判据**
   > （"影子那一格必须由本表显式提供"）。⇒ 留在那里的是**烘图侧与判据需要的文本**，
   > 不是"Bevy 宿主还活着"。
5. 记终账：J5 的五个读数（冷编 / 改一行 / 产物 / 到设备 / 到管线全就绪）。
   > ⚠ **这一条没有兑现**（S8-c 核出，不是读数的更正）：J5 的五格只量过一次，在 §108.2 的 S0 表里
   > （冷编 26.8 s ／ 改一行 1.3 s ／ 产物 10.19 MB ／ 到设备 570/548/557 ms），**剥离之后没有重记**；
   > 而那次用的仪器（`target/oracle/s0-coldbuild.ps1`、`target/oracle/s0-build.txt`）随 `target/oracle/`
   > 一起没了。⇒ 这是一笔**明账**，不是"已经记过"。
...
- **别动 `px_render` 的行为**：它是锚，J1–J3 全靠它。改它之前先问"我还要不要那张旧图"。
  > ⚠ **已被 §154 取代**（S8-c 标注，原句与读数一字未删）：`px_render` 已删 ⇒ 这条禁令没有对象了。
  > 但**它问的那个问题仍然是对的，只是换了对象**：动 `art/anchor/` 下那六张图或六份冻产物之前，
  > 先问"我还要不要那张旧图" —— 因为那六样是**判据本身**（`art/anchor/README.md`）。
  > ⚠ 两步的次序：§125 先把锚退回成"**冻结的 oracle**（不再演进）"，§154 才删它 ——
  > 从 §125 起它就已经不许动了，只是那时还能"提问"。
...
> ⚠ **这一格"锚 exe"已不存在，且找不回来 —— 已被 §154 取代**（S8-c 标注，表格与读数一字未删）：
> `target/oracle/` 在 2026-09-18 整个丢失，那支 exe 是**冻结的构建产物**，重建出来的不是同一个
> 字节序列（`art/anchor/README.md`、`art/anchor/hashes.txt` §五）。⇒ 这个 sha256 从"可复现的读数"
> 降级成"**记录**"：它证明过当时量的是哪一支，但今天谁也拿不出同一支来。
> ⚠ 同一场丢失里**活下来的**是六张判据图（当时也在 `target/j1/` 留了一份）与六份冻产物
> （内容寻址，`--no-frame-graph` 重烘后**文件字节逐字相同**）⇒ 它们进了仓库 `art/anchor/`，
> 成为今天判据的对手方。
...
> ⚠ **后半句已被 §154 取代**（S8-c 标注，原句与读数一字未删）：**"换 exe 就要重新确认"这条路
> 永远关掉了** —— 锚 exe 已丢失且不可重建（`hashes.txt` §五）。前半句仍然准确：
> 它记录的正是"这五个数是从哪台机器、哪个后端、哪一支 exe 上取的"。
> ⚠ 今天判据的绑定点换成了「这台机器 + Vulkan + **仓库里那六张图**」（`art/anchor/`），
> 而"换机器要重新确认什么"也换了：不是 exe，是**盘上字节的行尾约定**（`hashes.txt` §六）。
...
> ⚠ **已被 §154 取代**（S8-c 标注，原句与读数一字未删）：那支 exe 已丢失且不可重建
> （§108.1 那条标注、`art/anchor/README.md`）。**这一节的三格读数仍然有效**，而且它证明的事情
> （"锚 / CAS / 场景产物键在两次取数之间一个都没漂"）今天仍然**由构造保证**：
> 六张判据图在仓库里，J1 每次都对它们比（§154 判据，S8-c 复跑仍绿）。
> ⚠ 只有"靠同一支 exe 再复核一遍"这个**动作**没了 —— 它是这一节的方法，不是它的结论。
...
每个文件都是，`art/docs/archive/render-wgpu.md` 自己也用注释体写规格。
...
> ⚠ **这一节的"两个宿主"只剩一个了 —— 已被 §154 取代**（S8-c 标注，正文与读数一字未删）：
> 第二个宿主（Bevy 锚）已删 ⇒ "两个宿主的容器同形"这句话今天**没有对象**。
> ⚠ 但它当时要排掉的那个可能**已经由结果结清**：S2/S3 的判据后来全绿（§145 六档全绿），
> "无论像素多对都必然失败"这件事没有发生。
> ⚠ 表里那一行**不是死引用**：锚那张 `orbit-bare-nolight.png` 就是今天仓库里的
> `art/anchor/orbit-bare-nolight.png`（`7BBB18CE3612D4F7`，215193 B）⇒ 容器那几栏随时能重新量。
...
> ⚠ **这条修法已被 §154 取代**（S8-c 标注，正文与两个 sha256 一字未删）：没有 `px_render` 可重建了，
> 那支 exe 也丢了。⚠ 但**这一节要立的规矩是整篇里最该活着的一条**：
> **"判据跑的是哪个产物"本身就是判据的一部分**（§122 那条）。今天它以别的形式在跑：
> 每次重烘现取产物路径（§146.3②）、探针脚本头那三段事故记录、`hashes.txt` 顶上的"量法"。
> ⇒ 读这一节请读那条规矩，别读那段脚本。
...
> ⚠ **"冻在 `target/oracle/`"已被取代**（S8-c 标注，正文与表格里的六格读数一字未删）：
> 那份随 `target/oracle/` 在 2026-09-18 一起丢了，后来用 `--no-frame-graph` 重烘，
> **文件字节与表里的登记值逐字相同**，现在住在 **git 里的 `art/anchor/frozen/`**。
> ⚠ 这一节下面那两段推理（"`target/` 不入 git，挡不住 `cargo clean`"⇒"加一条硬判据"）
> **正是那次丢失的预言 —— 它自己应验了**；而立的规矩（"一份能被删掉的判据，不是判据"）
> 也因此在 `art/anchor/README.md` 里成了这个仓库的规矩。
...
> ⚠ **"冻在 `target/oracle/`"这半句已被取代，后半句被事实确认**（S8-c 标注，原文一字未删）：
> `target/oracle/` 那份锚图没了，**活下来的是当时恰好也在 `target/j1/` 留的那一份**
> （`art/anchor/README.md` 记了这场丢失），今天住在 `art/anchor/*.png`。
> ⇒ "丢的是可复现性，不是判据本身"这句**判对了**：图还在，J1 每次照跑（§154，S8-c 复跑仍绿）。
...
> ⚠ **"编一份 Bevy 宿主"这条前提已被 §154 取代**（S8-c 标注，原句与读数一字未删）：那支宿主已删、
> 锚 exe 已丢失，今天不会再有"必须编一份 Bevy 宿主"的活。⚠ 但这一节里**另一条仍然有效**：
> 这块盘的 `target/` 曾被撑满 ⇒ **别 `cargo clean`**（判据靶子在 `art/anchor/`，而**仪器**仍在
> `target/` 下，删了就没了 —— `art/anchor/README.md` 的"仪器留 target/"那条规矩）。
> ⚠ 顺带一个**由 §154 变得更干净**的读数：`cargo test --workspace` 今天**不再在那两支 crate 上炸**
> —— `px_render` 没了、`px_probe` 只依赖 `px_shader`（§154 的 A 案），九个 crate 全绿（§155.6）。
...
> ⚠ **这台仪器已经跑不了了 —— "向 oracle 提问"的能力随锚 exe 一起没了（§154）**
> （S8-c 标注，表格与十条读数一字未删）：`target/rings/` 与 `target/debug/px_render.exe` 都已不在，
> 而那支宿主不可重建（`art/anchor/hashes.txt` §五）。
> ⚠ **但下面这张表仍然是判据的来源**：它定出的次序规则（`depth_bias` 主键 + `objects[]` 反序）
> 已经落进烘图侧，并且**由六档逐字节回归替它背书**（§144/§145）。⇒ 记录留下、提问能力没了 ——
> 这正是 `hashes.txt` §五 要写清的那种损失。
...
> 判据出处：`art/docs/render/pass-table.md` §86.1 / §86.4。仪器：`target/j3/run.ps1`（烘→出图→记哈希）、
...
> ⚠ **表里那两支 exe 今天都不在了 —— 已被 §154 取代**（S8-c 标注，表格与拒词一字未删）：
> 结论因此更硬：帧图文档今天**只有本宿主能渲**，连"另一支能读什么"都不必再问。
...
> ⚠ **"Bevy 锚"那一侧今天是记录值，取不回来了**（§154；§153 末尾"我**没能**做的"已经写明）：
> 这一节的 pair 差是**记录读数**，不是同时测的两组。⚠ 本单元**没有**重测它，也不该假装能重测 ——
> 要重新取一组锚读数得先重建 Bevy 那支宿主并跑它那套 60 帧协议（超出本单元范围）。
...
> ⚠ **这一段今天没有对象了 —— 已被 §154 取代**（S8-c 标注，原句与三条测试名一字未删）：
> `cargo test -p px_render` 跑不了（crate 已删）。⚠ §154 专门记了这条的**读法陷阱**：
> **"一条红灯消失了"与"一条红灯被修好了"在报告里长得一模一样** —— 那三条红是**随 crate 消失**的，
> 不是被修好的。⚠ 同一格读数在 §153 末尾又出现一次（那里只列计数）。
...
## §149 J2 成立 —— `--sheet` 对照图（提交见下一笔）

### 判据（**我自己复现的**）

```
target\debug\px_render_wgpu.exe --offline --scene <现烘的 orbit-bare.pxart> \
    --out sheet.png --sheet --width 960 --height 640
A94F9F2D1437C06C ｜ 3840×1920 ｜ 3159948 字节   ✓
```

**两个形状都实测过**：老形状（锚 + 冻产物，本趟现出）与新形状（本宿主 + 现烘帧图产物）
**同一个哈希**，`--diff` = **0 / 7372800**。本宿主**读不了**老形状（结构性：`文档里没有资源 'scene_depth'`）
—— **是错误，不是静默错图**。

### ⚠ 最关键的一次 oracle 测量：格子像素依赖它在帧缓冲里的**绝对位置**

12 台**一模一样**的相机出 sheet：cell 0 与单独渲 `--cam 0,0,3.15` **逐字节相同（0 差异）**，
而 cell 1..11 与它差 **49–289 个像素**（最大通道差 2–5，全在剪影内部、确定的）。

⇒ **"12 次独立出图再拼"在逐字节层面可证是错的**。必须照 Bevy：**12 格共用同一批全尺寸中间纹理**，
每格 `set_viewport`；交还宿主目标那一条 **`set_scissor_rect`**（upscaling 的形状）。

⚠ 这条测量的价值：它**否掉了一条看起来更自然、而且能出"几乎一样"的图的实现** ——
**而"几乎一样"在这个工程里等于错。**

### 落地

`px_pass::Frame` 多一格 `viewport: Option<[f32;4]>`；落法由**纯数据函数** `cell_space_for` 定
（写外部目标 ⇒ 只 scissor；画 `size="view"` 池资源 ⇒ viewport；固定尺寸 ⇒ 整幅；
几何写外部目标 + 格子 ⇒ **当场拒**；`None` ⇒ **一个调用都不发**）。
⚠ 最后一行是关键：**新机制缺席时是"零调用"，不是"按整幅算一次"** ⇒ 单张那条路逐字节不变。

### ⚠ `px_render` 动了 2 行（机械），而处理方式是对的

`Frame` 加字段 ⇒ `px_render/src/passes.rs` 的结构体字面量必须补 `viewport: None` 才编得过。
**验了被钉住的锚 exe 没被动**：`target/oracle/px_render-bevy.exe` 仍是 **`D7ED54FDB8323EDD…`** ✓
⇒ 这才叫"没动锚"：**不是"我没改那个文件"，是"被钉住的那个字节序列没变"。**

> ⚠ **那一行读数今天取不到了 —— 已被 §154 取代**（S8-c 标注，原句与 sha256 一字未删）：
> 锚 exe 已丢失（`hashes.txt` §五），而且 §154 之后 `px_render` **整个 crate** 都没了 ——
> 那是个比"动 2 行"大得多的动作，路径是 §105 的 S8（用户裁过、判据全绿之后才做）。
> ⚠ 而**这一节要立的规矩一个字都不用改**：**"没动锚"不是"我没改那个文件"，是"被钉住的
> 那个字节序列没变"。** 今天这条规矩的载体是 `art/anchor/`（六张 PNG + 六份冻产物）：
> 它们进 git，每次重烘重出**逐字节比**（§154 判据，S8-c 复跑仍绿）。

### ⚠ 我的数又错了（第十次）

brief 里我写"相机表 **24 条**、12 个唯一 tag"—— **实测 12 条、12 个唯一 tag**。
错法：**我用正则在整个文件里数 `"tag"` 的出现次数**，而那张表在那份文档里出现了**两次**
（asset manifest 与 scene 各一份）⇒ 我数的是**出现次数**，不是**表里的条目数**。

⇒ 本 session 我自己的第十次错，形状与第九次不同：前八次"凭记忆写数"，第九次"没说清范围"，
**这次是"数了错的东西"**。规矩补一条：
**数一个集合的势，要数"那个集合里的元素"，不是"这个字符串在文件里出现了几次"。**

---

## §150 S7-a 常驻预览窗口（提交 `7b6970b`）

⚠ 这一节当时**漏了**（只存在于那条提交信息里），这里补上。

**判据**：窗口 `--shot` 与离线同文档/同尺寸/同机位**逐字节相同** ——
`orbit-bare 63184151909371A5`（=J1）、`orbit-soft FA20FAD37BC61EA2`（=J1）；
`--place 35,20,8` 之后窗口那张 == 离线 `--cam 35,20,8`（`B77A5452C88D31E2`）。
**结构性证法**：画面画进**本进程自己的** `Rgba8UnormSrgb`，回读的字节 ① 原样上传屏幕
（**两端都用非 sRGB 视图** ⇒ 屏幕字节 = PNG 字节）② `--shot` 原样写盘
⇒ **"屏幕上是什么"与"文件里是什么"只有一个数据来源**。

**鼠标**：投真窗口消息（**不动用户光标**），Δyaw/Δpitch = **0.006 rad/物理像素**（Bevy 常数）。
⚠ 抓到真缺陷：第一版把 Bevy 的**弧度**常数加到我们**度**的状态上 ⇒ 拖 100 像素只转 **0.6°**，
**看着像"鼠标没反应"**。**两套单位在同一个变量上相遇，而"没反应"会被当成"没接上"。**

**七个岔路全部批准**，其中最要紧的两条：窗口的 `(yaw,pitch,distance)` **与 `--cam` 是同一套数**
（Bevy 的窗口 pitch 正方向与它自己的 `--cam` 相反 —— 拒绝"同一个三元组两套含义"，
而这正是判据能成立的原因）；**删掉租约 ⇒ 窗口自己退出**（于是"关窗"有机械做法）。

⚠ 当时已知但未解决：**每帧 0.8–2.3 s**（§151 解决）。仍需人眼：窗口**真的显示**了那张图
（"送进交换链的字节 = PNG 的字节"之后那一跳看不见）、手感、拖动流畅度、改尺寸后的长宽比。

---

## §151 保留态：一次准备 + 逐帧画（提交 `dirty → 见下一笔`）

### 判据（reviewer 复现）

J1 六档与 `art/anchor/` **逐字节相同**；J2 `A94F9F2D1437C06C`（**保留态 `--time 2` 也是**）；
J3 四条 ✓；逃生门六份**文件字节** ✓。测试 `px_pass` 32｜`px_render_wgpu` **74**｜`px_shader` 20
｜`px_protocol` 20｜`px_graphs` 12 —— 0 failed。

### 帧时（reviewer 自量，`--time 6`）

| 档 | 改前每帧 | 准备（一次） | 第 1 帧 | **第 2..6 帧中位** |
|---|---|---|---|---|
| `orbit-bare` | 731 ms | 695.3 | 37.9 | **5.1** |
| `orbit-bare-shadow` | 758 | 743.9 | 50.0 | **11.7** |
| `orbit-soft` | 1066 | 981.1 | 100.9 | **13.7** |
| `orbit-proxy-fine-bound` | 1943 | 1854.9 | 94.3 | **9.6** |

⇒ 每帧 **~100–200×**；而**"准备"≈ 改前每帧** —— 那正是"每帧重付一次准备"的直接证据。
一次性那条路 **+0.5–0.9%**（同分钟 A/B，没变慢）。

### ⚠⚠ 它推翻了派活所依据的诊断

**"没有管线缓存"只对了一半**：`px_pass::Executor` **本来就有**按内容键的管线表，
**缺的只是让执行器活着**；而建管线只占 731/1066 ms 里的 **28/77 ms** ——
**大头是装载**（`planet` 网格解码 + 审计 **529 ms**）。
⇒ 收益来自**留文档**，不是留管线。

⚠ **账记在 reviewer 头上**：他拿着一个**从没量过的诊断**去划了单元边界。
实现方没有照着边界做，而是**先量切分**。这是本 session"reviewer 带的判断被实测纠正"的**第六次**，
共同形状是：**把"读到的"当成"量过的"。**

### 边界（判据：这是谁的函数）

文档（+CAS+`pcg_root`）→ **保留**；尺寸/格数 → **只在"层身份"变时重建**
（层身份 = 目标尺寸 + 每格绝对矩形，**相机不在里面**）；相机 → **每帧两次 64 字节 `write_buffer`**；
录命令/提交/回读/清目标 → 每帧。

**三处被迫改形状**：① 视图 uniform 要留着 ⇒ 用途需 `COPY_DST`，而
**`DeviceExt::create_buffer_init` 不会替你加**（一手核过 wgpu 29.0.4 `util/device.rs:40`）；
② **宿主目标每帧先清** ⇒ "复用那张"**按字节等于**"新那张"（判据因此仍成立）；
③ 从文档推出来的 `loads_before_write(plan)`（今天对全部判据文档为空，四个单测钉着）
—— **不是"我知道今天不会响所以不管"，而是"从文档推出来并钉住"，且保守方向选对了**（响了只是慢，不会错）。

### 窗口

`orbit-soft` 第 1 帧 1176 ms → 拖动每帧 **33–46 ms（中位 42）**；改前 1066–1160 ms/帧。
⚠ 实现方自己抓到一个**假 ✗**：拿 `--where` 的 **4 位小数**喂 `--cam` 得到**另一台**相机
⇒ 改成读 `viewer-camera.json` 的**完整 f64**。**§146.3③ 那一族的第六次**
（一条读数丢掉了它需要的精度，于是"错了"看起来像"东西坏了"）。

### 没跑的（口径）

`px.ps1 -Target test`（36 套）、`harness.ps1`、`frame-probe.ps1`、一次 `--serve` 往返。
⚠ **服务那条路故意没动**（仍是每条请求 `open`+`draw`）—— §147.2 的 **R 判据**靠的正是
"按构造没有污染源"；把保留态混进服务会让 R 判据失去证明力。**这个取舍是对的。**

---

## §152 S7-b 热重载成立（提交见同一笔）

**判据**：改一行 `.wgsl` 存盘 ⇒ 画面变。**量图**（每帧回读字节的 sha16 + 墙钟），正反两向各三轮：

| 次序 | 落盘 → 发现+重载 | 落盘 → 画面 | ≤1 s |
|---|---|---|---|
| 空机器 | 中位 **114 ms**（97–135） | 中位 **272 ms**（228–288） | **6/6** |
| 紧接着回归集之后 | 中位 **207 ms** | 中位 **509 ms**（453–890） | **6/6** |

改过去 `a8e1525c9dbcb69c`，改回来 `d256dac6bf281aaf`（逐字节回到开窗基线），
反向那轮窗口存图 = **`FA20FAD37BC61EA2` = 离线 J1 的 orbit-soft**。
**负对照**：只改注释 ⇒ 画面不变（仍报作废 1 槽）；`touch` ⇒ 明说"字节没变 ⇒ 不重载"。
**写坏一行** ⇒ 当场拒 + **画面留上一版**。**改库** ⇒ `作废 3 槽｜跳过 2 槽`（量出来的）。

### 最值钱的一步也是量出来的

第一版一次改动把**每一槽**都重组装 ⇒ 重载 200–370 ms、判据 **5/6**；
按闭包指纹跳过（指纹与原文哈希都没变 ⇒ 组装结果不可能变）⇒ 重载 **97–125 ms**、判据 **6/6**，
读数里多出"**跳过 4 槽**"证明它真在跳。
⚠ **一个没有读数的优化，无法与一个没生效的优化区分。**

### 边界

只有"组装后的 WGSL"（5 槽）会作废；网格/贴图/参数块/实例数组/宿主目标/**执行器**一个都不动。
四道门任一不过整批不落：naga 校验 / 反射契约逐字节不变 / 片元入口还在 / 整份 `Plan::check()`（在克隆上）。
"来源"是装载时验过的事实，验不过就拒并**指出第几行不同**。

### 四条岔路的裁决

1. **顶点阶段不能热重载 —— 这不是缺口，是架构。** 它们在文档里是**内联全文**（帧配方烘进文档）
   ⇒ **文档才是真相**，改它们要**重烘**；给它加 `PassSpec` 来源名 = 给"已烘进文档的东西"再开一条外部真相 ⇒ **不加**。
2. **不给 `px_pass` 加"新建了几条管线"的只读出口** —— 现有代理读数够判据，对外面该保持最小。
3. **轮询保持 200 ms** —— 余量够（最坏 890 < 1000）。⚠ **判据过得去就不调它**；
   但记下"第一个该动的杠杆是轮询"（1 秒预算 = 轮询 ≤200 + 重载 97–207 + 一帧 35–111）。
4. **`--image-hash` 在非窗口路当场拒** —— 对，与 `--time` 同一条规矩：收下不说就是"说了没做"。

---

## §153 J4 计时对照（逐条 pass 的 GPU 时间戳）

**这一节量的是一个新仪器，不是一次调优。** 判据是"分组 + 量级"，而**仪器本身要先证明它在出数**。

### 判据与结果（一句话）

分组**对上**了；量级上 `{soft, proxy}` 与锚那三个数在 20% 以内，
`rings` 只到锚的 1/14（0.013 vs 0.16 ms）但**两边都是"很小"那一档**，而且它比"口径差"还小
（见下文"口径差那一栏"）。**逐条 pass 的读数不是 Bevy 的 `gpu_ms`，名字也不叫 `gpu_ms`。**

### 仪器是什么（说清"量的是哪个量"）

`px_render_wgpu --spans 预热,测量,轮数`：在保留态那条路上，**逐条 pass** 写 GPU 编码器级时间戳。

| 字段 | 定义 | 与 Bevy 的关系 |
|---|---|---|
| `inside_*` | pass **里面**第一条/最后一条命令之间 | **同一套边界**（Bevy `internal.rs:455-478` 也是 `pass.write_timestamp`） |
| `envelope_*` | 含 `vkCmdBeginRenderPass`/`EndRenderPass` 的开/关 | **口径差**，不是渲染耗时 |
| `matched_inside_*` | 上面那些**在 Bevy 侧查得到对应段**的 pass 之和 | 与 Bevy 七段**唯一**能对的子集 |
| `frame_*` | 含宿主目标清屏的整帧 | Bevy 没有等价的一段 |

映射表（**政策，不是推导**；出处逐条在 `px_render_wgpu/src/spans.rs::SPAN_MATCH`）：
`prepass`→`early prepass`；`opaque`+`sky`→`main_opaque_pass_3d`（Bevy 天空盒画在**同一条 pass** 里
⇒ 我们是两条 pass，多一次附件 load/store，**这是结构差**）；`transparent`→`main_transparent_pass_3d`；
`blit`→`upscaling`；`copy_depth`→**查过了，Bevy 没有**（它自己的深度拷贝在 `pass_span.end()` **之后**）；
`point_shadow` 与那一族 `point_shadow_{灯号}_{面}`→**查过了，Bevy 没有**（只有 CPU `info_span!`
⇒ **Bevy 的 `gpu_ms` 不含影子图**）；Bevy 独有的三段 `bin_unpacking`/`early_mesh_preprocessing`/`clustering`
**我们一条都不跑** —— 它们是**配对差里的公共项**，配对时相消（这一点是**假设**：我拿不到那三段在两种内容下的
对象/灯数，**没有把它界住**）。

### 读数（2026-09-19 10:49，两次独立批次一致）

协议：**一个进程里四档全测、每档只 `Session::open` 一次、按轮交错**；5 轮 × (8 预热 + 24 测量) 帧/轮；
960×640 与 2240×1400 各一次进程。整批 **32 s**（`open` 8 次共 ≈8.5 s）。
⚠ **"第 1 轮不算"这条规则是在跑之前定的**（第 1 轮含 `open` 留在队列里的上传工作 + 时钟爬升）。

| 档（2240×1400） | 匹配子集 p50 | 同轮配对差 Δ（第 2..5 轮） | **锚（记录值）** |
|---|---|---|---|
| `orbit-bare` | 0.3062 | — | — |
| `orbit-rings` | 0.3195 | **+0.0133**（0.0092…0.0205） | **+0.16** |
| `orbit-soft` | 7.0564 | **+6.7512**（6.7010…6.7543） | **+8.12** |
| `orbit-proxy-fine-bound` | 8.5514 | **+8.2452**（8.2319…8.2565） | **+8.29** |

- **分组**：`rings ≪ {soft, proxy}` —— 两边一致；`soft < proxy` 的次序也一致。
- **量级**：soft −17%、proxy −0.5%、rings −92%（绝对值 −0.15 ms）。
- **锚 2240×1400 的绝对值**：锚的 `gpu_ms.p50` 是**七段之和**，其中三段我们不跑 ⇒ **绝对值不可直接比**，
  能比的只有**配对差**（这正是 §147 修订后判据的形状）。
- 960×640 同一套协议：bare 0.1536｜rings +0.0031｜soft +2.6716｜proxy +3.7775（量级随像素数缩放，分组不变）。

### 仪器的噪声底（这是**读数**，不是要抹平的脏）

- **跨轮漂移**（全 5 轮极差）：bare 1.13 / rings 1.20 / soft 4.11 / proxy 2.72 ms
  —— 比 `rings − bare` 那个差（0.013）大 **~100×**，与 §147.4"参照图自己漂 4.5×"同族。
- ⇒ **对照必须在同一轮之内配对做**（锚的 `pair` 本来就是同一个会话里先后测的差）。
  配对之后：rings Δ 的极差 0.0113、soft 0.0533、proxy 0.0246 ms ⇒ 差值可分辨到 ±0.01 ms 量级。
- **第 1 轮 vs 第 2..5 轮**：bare +1.13、rings +1.19、soft +4.06、proxy +2.71 ms
  —— 这就是"**准备那一档在 GPU 时间戳这一侧的尾巴**"（§151 把准备/逐帧劈开的那条界线）。
- ⚠ 这台机器上**别的单元在跑 `cargo test`**（用户报"总有一个测试把 CPU 跑满"，主代理核实过）。
  上面每一轮都带着这个外来负载取的 —— **照实报，没有为了好看重跑**。

### 口径差那一栏（**要求单列**，它不是渲染耗时）

`envelope − inside`，2240×1400、第 2..5 轮中位：

| 档 | 匹配子集 inside | 包络 | **口径差** |
|---|---|---|---|
| bare | 0.3062 | 0.3982 | **0.0920** |
| rings | 0.3195 | 0.4126 | **0.0931** |
| soft | 7.2663（含影子） | 7.3738 | **0.1075** |
| proxy | 8.5514 | 8.6364 | **0.0850** |

逐条 pass 上，包络比 pass 内多 **0.000–0.011 ms**（`prepass` 最大 0.0103）。
⇒ **`rings − bare` 的 +0.013 ms 比"边界约定本身值多少"（0.09 ms）还小**：
  rings 那一档**不可能**被这个仪器分辨到 0.16 ms 的精度上；它是一个"两边都小"的结论，不是一个"对上了"。

### 三条实测的坑（都是"仪器能把自己拆掉"那一类）

1. ⚠⚠ **没写过的格 + `VK_QUERY_RESULT_WAIT_BIT` = GPU 挂死 ⇒ 设备丢失**。
   第一版给每条 pass 排四格（copy 也排），而 copy 只写得起两格 ⇒ resolve 等一个永不来的结果 ⇒
   `Error in Device::poll: Validation Error / Parent device is lost`，
   **之后连新建的缓冲都变成 invalid**（报错指向一个与被量对象无关的缓冲，查了很久）。
   修法：排布**不许有没人写的格**（copy 只排两格），而且那张排布**按形状算一次**
   （`px_pass::frame_slots`），**写的人和读的人用同一张**。
2. ⚠ **resolve 必须与那些 `write_timestamp` 在同一条命令缓冲里**。更省事的形状
   （所有帧写进同一张查询集、跑完另起一条提交去 resolve）读到的是**全 0**
   （实测 `--spans 0,1` 与 `--spans 2,3` 都是"非零 0/32 格"）。现在的形状是**每帧帧末就地 resolve**
   到自己的那块区域（查询集因此只需要一帧那么大）。
   ⚠ 记账：这条是**实测 + 一处 wgpu-core 源码**（`command/render.rs:2291-2300` 的 `pending_query_resets`），
   我**没有**把 wgpu 29 多命令缓冲架构下的提交次序完全读通。
3. **`MAP_READ` 只许与相反的那一个 COPY 组合** ⇒ resolve 那块（`QUERY_RESOLVE|COPY_SRC`）与
   回读那块（`COPY_DST|MAP_READ`）**必须是两块**。

### 一条"要不要为它改名"的边界（`--spans` 的代价）

`--spans` 那条路**每帧都回读像素**（与不带它时同一条路）⇒ 它比"真正的帧循环"多一次同步；
Bevy 那 60 帧是**流水线**里的。**这一条是仪器差**，写在这里以免有人把两个数当同一个。

### 不变的部分（回归集重测）

改后重跑 `target/j4/regress.ps1`：**失败格数 0** —— J1 六张与 `art/anchor` 逐字节、
J2 `A94F9F2D1437C06C`、J3 四条、`grade_half` `8DECA8C60117989A`、逃生门六份与 `art/anchor/frozen`
**文件字节**逐字节。**没有例外清单。**
⚠ 这一遍是在**最终那支 exe**（sha256 `C957CC0255F60629`）上重测的：
中途清掉两处告警（`spans.rs` 里一个没人用的常数、`main.rs` 里一个多余的 `mut`）后重编过，
而那之后**判据重跑了一遍**（"改了就得重测"不是"这次改的无关紧要"）。
测试：`px_pass` 34 ✓（较改前 +2：新增的两条时间戳判据）、`px_render_wgpu` 77 ✓、
`px_shader` 20 ✓、`px_protocol` 40 ✓、`px_graphs` 22 ✓；
`px_render` 23 ✓ / **3 ✗（预存红，一行未动）**。

### 计时：改前 / 改后（都要求）

`target/j4/time.ps1 -Which before|after`（改前那支 exe 保在 `target/j4/before/`，sha256 `BDF7A226CB57A6E3`）：

| 档 | 准备 改前→改后 | 第 2..6 帧中位 |
|---|---|---|
| `orbit-bare` | 712 / 700 → 746 / 742 | 5.3 → 5.8–6.3 |
| `orbit-bare-nolight` | 707 / 725 → 699 / 747 | 5.2 → 5.8 |
| `orbit-bare-shadow` | 714 / 752 → 727 / 738 | 10.8 → 11.8 |
| `orbit-rings` | 702 / 741 → 727 / 745 | 5.4 → 6.1 |
| `orbit-soft` | 990 / 1036 → 1034 / 1047 | 14.1 → 14.5–14.9 |
| `orbit-proxy-fine-bound` | 1863 / 1900 → 1969 / 1893 | 9.8 → 10.4 |

⚠ **两个数一列就是这么读的**：`改前(第一次) / 改前(顺序倒过来那一次) → 改后(同两次)`。
顺序倒过来之后差值**换号**（`nolight` 改后更快 26 ms、`proxy` 改后更快 76 ms）⇒
那一档差是**会话内的先后效应**，不是这两支 exe 的差。
**交替测准备那一笔**（同一分钟内 before/after 各 4 次）：bare 中位 689.2→697.2（+8.0）、
soft 982.1→977.2（−4.9）、proxy 1844.5→1851.5（+7.0）⇒ **±1% 以内，没有系统性变慢**。
一次性那条路（`ab-oneshot.ps1`，四趟交替）：bare 1321/1328 → 1328/1324；
rings 1358/1339 → 1397/1355；soft 1668/1688 → 1667/1677 ms ⇒ **在 ±1–3% 的轮间抖动里**。

**拖动那条路**（`drag.ps1 -Which before|after`，`orbit-soft`、真窗口消息、窗口自己打的"画+上传"）：

| 次序 | 改前 | 改后 |
|---|---|---|
| 第一趟（改前先） | 中位 15.0（14–47） | 中位 34.0（14–50） |
| 第二趟（改后先） | 中位 67.0（47–71，只有 8 帧） | 中位 15.0（14–49） |

⇒ ⚠ **这台仪器在现在的外来负载下分辨不了这两支 exe**：中位在 15–67 ms 之间随**跑次**摆，
而**最小帧两边都是 14.0 ms**（§151 记的"拖动中位 33–46"落在同一带里）。
**照实报，不挑那一次好看的。** 四趟的"窗口相机 vs 离线 `--cam`"都是**逐字节相同**（那条判据没有退化）。

### 我**没能**做的（口径，别当成做了）

- 锚那三个数是**记录值**，**取不回来了**：锚那支 exe 已经不在了（§147 之前就没了），
  `px_render` 的源码还在（拆掉之前都在）——**要重新取一组锚读数，得先重建 Bevy 那支宿主并跑它那套 60 帧协议**，
  而那件事超出这一单元的范围。⇒ 这一节比的是**记录读数**，不是同时测的两组。
  > ⚠ **§154 之后这半句要改读**（S8-c 标注，原句一字未删）：源码**不在工作树里**了，
  > 但**在 git 历史里**（`git show f121ee3^:px_render/src/main.rs`）⇒ 引用仍可查、重建在原理上仍可能，
  > 只是重建出来的**不是**当初那支 exe（冻结的构建产物 —— `art/anchor/hashes.txt` §五）。
- 我**没有**把 Bevy 那三段（`bin_unpacking`/`early_mesh_preprocessing`/`clustering`）界住，
  只用"配对差里它是公共项"这条**假设**。要界住它，得拿到两种内容下 Bevy 侧的对象/灯数。
...
换回旧桩表跑：**同一个 check、同一个数**（`2.2587842e-1`），与 `docs/render/renderer.md:637-638`、
`docs/archive/handoff.md:299` 登记的已知红逐字相同。§46.3 的 arbiter（`--bin field_dual`）全绿。
...
## §156 S8-c：**标注取代、读数一字不删**（本单元；提交见下一笔）

> 用户口径（本单元的工单）：
> **被推翻的规则加一行「已被 §X 取代」；只有「读数已作废且无处引用」的才删，删前把清单过给 reviewer。**
> ⇒ 本单元**一行读数都没删**：19 个文件、**全是原地插入** —— `git diff --numstat` 逐文件
> **deleted = 0**（逐文件的加行数在 §156.6）。⚠ 唯一的"新"东西是**新增**：这一节、`art/anchor/hashes.txt` §六、
> `art/anchor/README.md` 那三行指针、以及几处源码注释里的标注。
>
> ⚠ 同样重要的一条边界：**`art/shaders/` 与 `art/frame/` 一个字节都没动**（§155.4 第 1 条：
> 注释进模块源码 ⇒ 进闭包 ⇒ 换键）。判据：`git status -- art/` 只有 `anchor/README.md` 与
> `anchor/hashes.txt` 两份（它们**不在任何产物里**，所以可以从这里改）。

### §156.1 这一轮就地标了什么（被取代 → 取代者）

| 处 | 被谁取代 | 一句话 |
|---|---|---|
| §100 目标那句「先与今天的 `px_render` 同图，再取代它」 | §154 | 两半都兑现了 |
| §100「不许删 `px_render`」 | §154 | 判据全绿之后删了；靶子移交 `art/anchor/` |
| §101 J6「`cargo test -p px_render` 照旧」 | §154 | 那条命令跑不了；意图 = 九个 crate 的测试 + 回归集 |
| §102 那些 `px_render::…` 落点 | §154 | 搬到 `px_render_wgpu::*` / `px_shader::*` |
| §103.1「两个宿主必须同时活着」 | §154（经 §125） | 第二个宿主已删；桩表作参数的设计仍在 |
| §103.1「改这一处必须证明 Bevy 宿主没变」 | §154 | 证明动作做不了；等价物 = J1 对 `art/anchor/*.png` |
| §103.1「改名是最后一步」 | §155 | 决定不改名（改名会换掉所有键、像素不动） |
| §105 S8-2 改名 | §155 | 同上 |
| §105 S8-3「重烘重出 J1/J2/J3 逐字节相同」 | —— | **照旧有效**，只是对手方换成仓库里的六张图 |
| §105 S8-4「删 bevy 桩表」 | §155 | 只删了 crate 与 `slots.rs`；桩表还有真实用户 |
| §105 S8-5「记终账 J5」 | —— | **没兑现**（见 §156.5），本单元只标注、不补数 |
| §107「别动 `px_render` 的行为」 | §154 | 对象换成 `art/anchor/`，**问法不变** |
| §108.1 锚 exe 那一格 | §154 | 文件已丢失且不可重建；读数降级成**记录** |
| §108.1「换 exe 就重新确认」 | §154 | 那条路永远关掉；今天要确认的是**行尾约定** |
| §113「同一个锚 exe 的复核」 | §154 | 三格读数仍有效；只有"再复核一遍"这个动作没了 |
| §119「两个宿主的 PNG 容器同形」 | §154 | 第二个宿主没了；它要排掉的可能已由结果结清 |
| §122「先 `cargo build -p px_render` 再验」 | §154 | 修法没了；那条规矩（判据跑的是哪个产物）最该活着 |
| §125 六份冻产物「冻在 `target/oracle/`」 | 仓库 | 那场丢失应验了这一节的预言；今天在 `art/anchor/frozen/` |
| §138 环境「编一份 Bevy 宿主会链接失败」 | §154 | 那件活没了；「别 `cargo clean`」仍然有效 |
| §143「问 oracle 本人」那台仪器 | §154 | 仪器与"提问"的能力都没了；十条读数仍是规则来源 |
| §146.7「`cargo test -p px_render` 3 条预存红」 | §154 | 那三条红是**随 crate 消失**的，不是被修好的 |
| §147.3① 两支 exe 的表 | §154 | 两支都不在了；结论更硬（帧图只有本宿主能渲） |
| §147.4「Bevy 锚」那一组读数 | §154 | 记录值，取不回来（§153 末尾已写明） |
| §149「验了被钉住的锚 exe 没被动」 | §154 | 那个读数取不到了；**"没动锚"的定义不变** |
| §153「`px_render` 的源码还在」 | §154 | 不在工作树，在 git 历史（`f121ee3^`） |

**六份别的文档**（同一轮，就地插入，正文一字未删）：`docs/archive/handoff.md`（顶上一条横幅 + 9.6/9.7
两条死指针）、`docs/render/schema-limits.md`（横幅）、`docs/render/pass-table.md`（横幅 + §86.1 P 那一格与 §89 落点）、
`docs/archive/bevy-exit.md`（**保留声明**：它是离开 Bevy 的证据链）、`docs/art/clouds.md` / `docs/render/renderer.md` /
`docs/render/instruments.md`（**只加一条"引用承重、不许删"的横幅**，正文不动）。

### §156.2 ⚠ 怎么读两种「出处指针」（这一轮核实过，两类都不许删）

| 指针 | 今天在哪 | 还可查吗 |
|---|---|---|
| `bevy_pbr::…` 与 `bevy_pbr-0.19.1/src/…:行`（`06-clouds` / `08-renderer` / `09-instruments` / 本篇 §110.2 那些） | cargo registry：`~/.cargo/registry/src/index.crates.io-1949cf8c6b5b557f/bevy_pbr-0.19.1` 等 | **可直接打开**。本单元实核四例：`bevy_pbr-0.19.1/src/material.rs` 的 `.insert(3, …)`／`bevy_core_pipeline-0.19.1/src/core_3d/mod.rs:426-449` 的 `sort_by_key`／`bevy_light-0.19.1/src/lib.rs:305-306` 的 `#[default] Gaussian`／`bevy_pbr-0.19.1/src/render/mesh.rs:3631` 的 `cull_mode: Some(Face::Back)` |
| `px_render::…` 与 `px_render/src/…:行`（本篇与源码注释里那些） | 工作树里**没有**；git 历史 `f121ee3^` 里有 | **可查，但换了工具**：`git show f121ee3^:px_render/src/main.rs`（本单元实核过 `main.rs:131-132` 的 `GRID_COLS/GRID_ROWS`、`:355` 的 `quantile`、`art_cache.rs:477`、`scene.rs:233`） |

⇒ 两类都**不许删**：`bevy_*` 那类回答"**这个数为什么是这个数**"，`px_render/…` 那类回答
"**这段逻辑从哪抄来**"。⚠ 区别只有一个：**取法不同**（一个在 registry，一个在 git）。
⚠ 第三类要留意：`art/shaders/**` 里的 `#import bevy_pbr::…` 是**出处指针**（§155.4），
它**在产物里**，所以连注释都不许改。

### §156.3 回归集（S8-c 重量：标注之后的第一遍）

```
量法（仪器 target/s8c/regress.ps1，逐条照 art/anchor/hashes.txt 的"量法"）：
  shaders/planet/clouds 重烘 → 六档 scene 现烘（老形状 + 帧图形状各一遍）→
  出图 960×640 不给 --cam → 与 art/anchor/*.png 逐字节；--sheet 出 3840×1920；
  --bin passes 四份配方出图；老形状那六份比**文件字节** sha256
```

| 判据 | 读数（S8-c，2026-09-19） |
|---|---|
| J1 六档 | `63184151909371A5` / `7BBB18CE3612D4F7` / `C03FFF3235264DD5` / `B5799E4F1649535C` / `FA20FAD37BC61EA2` / `32872F80AC867BE3`，**六格与 `art/anchor/*.png` 逐字节相同**（失败 0/6） |
| J2 `--sheet` | `A94F9F2D1437C06C`｜3840×1920｜3159948 B |
| J3 四条 | `none` `63184151909371A5`｜`invert` `733408200119C203`｜`invert_vignette` `745BE24FE1467192`｜`scratch` `2D61519C6544E3C1`（失败 0/4） |
| 逃生门六份 | **文件字节** `2795F948E6987E11` / `F60B19B0AE7E229F` / `1A27679882A10D6B` / `CB363DA74A90F63E` / `2C6C8592AD45214B` / `ACB824E9EC0BA893`，与 `art/anchor/frozen/*.pxart` 逐字节（失败 0/6）；**产物是刚烘的**，路径从 stdout 现取（不读清单） |
| 测试 | 逐 crate 一遍 + `cargo test --workspace` 一遍，**两条路都是 309 passed / 0 failed**：`px_pass` 34｜`px_render_wgpu` 77｜`px_shader` 21｜`px_protocol` 40｜`px_graphs` 22｜`px_probe` 3｜`px_ops` 25｜`px_verify` 3｜`planet_x` 84（`px_mc` / `game` 各 0 条） |

⚠ 这次标注**改了 Rust 源码注释** ⇒ exe 的 sha256 变了（`CD7FDF3690CAC6F2…` → `C03038C086DA2F16…`），
而**判据一格没动** —— 这正是 §154 那条读数的同一形状："**文本变了，数没变**"。
⚠ 这一轮**没有**覆盖的（说清边界，别当它绿）：`grade_half` 那格（§147 的 `8DECA8C60117989A`）、
J4 的计时、`tools/frame-probe.ps1` 的 `-Phase shot`。它们不在本单元的工单里，也**没有**重跑。

### §156.4 ⚠ 行尾约定：落进判据登记表，并量出 §154 那句话**不成立的那一半**

§154 原话：「**指纹与产物键哈希的是"盘上那些字节"，而工作副本是 CRLF**
（`core.autocrlf=true`，index 是 LF）⇒ 换一台 checkout 约定不同的机器重烘，键会变、
逃生门六份文件字节也会变。……**进 S8-c。**」

本单元把它量完了，**结论：一半成立、一半不成立**（全文与量法在 `art/anchor/hashes.txt` §六）：

- **index 是 LF —— 成立**（逐文件按 blob 字节数核过：`surface.wgsl` 盘 9780 / index 9603，差 177 = 行数）。
- **"工作副本是 CRLF" —— 不成立**：同一棵树下**两种行尾并存**。内容入口与库是 CRLF
  （`surface` +177 B、`clouds` +777、`atmosphere` +53、`ring` +21、`lib/{common,light,noise}` +59/85/164），
  而 `art/shaders/{blit,px_grade,px_vignette}.wgsl` 与 `art/frame/{skybox,vertex_mesh,vertex_sky}.wgsl`
  **在盘上是 LF**（差 0）。
- **两个方向都会咬人**（用 `git checkout-index` 实测过"另一份 checkout 会给什么字节"）：
  换到**无转换的机器**（Linux / CI）⇒ 内容入口从 CRLF 变 LF ⇒ 闭包指纹变 ⇒ **逃生门六份破**；
  **在本机重新 checkout** ⇒ `art/frame/*.wgsl` 与三份 pass 入口从 LF 变 CRLF ⇒ 帧材质内容键变
  ⇒ 带帧图的文档字节变（图不变）。⚠ 后一条 `git status` **看不见**（两种行尾 clean 之后是同一个 blob）。
- **报警器**：`px_shader` 那条判据测试（四份入口的组装字节数 + FNV + 闭包指纹）读的就是盘上文本
  ⇒ 换约定之后它**先红**，而且说的是"字节数变了"。**别把它当噪声改掉。**
- ⚠ 这一格**只量了字节数**；"键会变"是 §155.1 机制的直接推论（指纹哈希的是源码字符串），
  本单元**没有**在另一份约定的副本上重烘 —— 那要动 `art/` 下的字节，是明令禁止的。
- ⚠ 今天全绿的前提因此要**说准**：不是"锚和我在同一种约定下量的"（锚 exe 已丢失，这句无法检验），
  而是两条**可检验**的：① 六份冻产物今天重烘**文件字节逐字节相同** ⇒ 内容入口的字节约定自冻结以来没漂过；
  ② 判据图不比文本字节（行尾不改像素，§155.3 已证"像素与文本无关"）。
- ⚠ 顺带一条**本单元自己踩到的**：改这些文件时，`git diff` 提示"LF will be replaced by CRLF"——
  那是**工作副本的行尾只有 git 之外的眼睛才看得见**的另一个侧面。本单元用一个探针
  （`target/s8c/eol-probe/`：造一份 CRLF、一份 LF，各改一处，再看字节）**证明了编辑工具按原样保留行尾**
  ⇒ 19 个改过的文件各自保持原约定（14 份 CRLF / 5 份 LF），**没有一份被这次标注改了行尾**。

### §156.5 本单元顺带核出的两笔明账（不是读数的更正）

1. **J5 的终账没有兑现**（§105 S8-5 那条标注）：J5 五格只在 §108.2 的 S0 表里量过一次，
   剥离之后没重记，而那次用的仪器（`target/oracle/s0-coldbuild.ps1`、`target/oracle/s0-build.txt`）
   随 `target/oracle/` 一起没了。⇒ 要么重记一遍，要么把这一格**明确作废**——但那是评审的裁决，
   本单元只把它标出来（**不许**默默当成"记过了"）。
2. **`px_protocol/tests/crate_graph.rs` 的禁止边今天没人在守**：表里第一条是 `("px_render","px_sim")`，
   而 `px_render` 已删；今天那支宿主叫 `px_render_wgpu`，**不在表里** ⇒ "渲染宿主不许拖 sim"这一格
   **目前是空的**。本单元只在常量上留了标注，**没有改门**（动门的覆盖面要评审裁）。

### §156.6 逐文件的加行数（`git diff --numstat`；**deleted 全为 0**）

> ⚠ **这一格是"一字不删"的机械证据**：19 个文件没有一个删过行（`deleted = 0`），
> 所以"读数还在"不是靠人读一遍，是靠 `git diff` 说出来的。
> ⚠ 量法：`git diff --numstat`（加行 / 删行 / 文件），before = `git show HEAD:<文件> | wc -l`。

| 文件 | HEAD → 现在（行） | +行 |
|---|---|---|
| `docs/archive/render-wgpu.md` | 3808 → 4104 | +296 |
| `docs/render/pass-table.md` | 187 → 200 | +13 |
| `docs/archive/handoff.md` | 717 → 730 | +13 |
| `docs/art/clouds.md` | 1386 → 1398 | +12 |
| `docs/render/renderer.md` | 698 → 710 | +12 |
| `docs/render/schema-limits.md` | 86 → 97 | +11 |
| `docs/render/instruments.md` | 602 → 616 | +14 |
| `docs/archive/bevy-exit.md` | 305 → 314 | +9 |
| `art/anchor/hashes.txt` | 62 → 126 | +64 |
| `art/anchor/README.md` | 57 → 62 | +5 |
| `px_shader/src/lib.rs` | 670 → 678 | +8 |
| `px_shader/src/assemble.rs` | 369 → 375 | +6 |
| `px_shader/src/reflect.rs` | 362 → 366 | +4 |
| `px_render_wgpu/src/viewer.rs` | 1857 → 1862 | +5 |
| `px_pass/src/lib.rs` | 5760 → 5763 | +3 |
| `px_graphs/src/frame.rs` | 1340 → 1344 | +4 |
...
| `docs/invariants.md` | 1 行：索引那一格补 §157 |
