...
## ⚠⚠ 最重要的一条：**动手前先确认浏览器在哪个实例上**

`dsh-browser` 是**跨 session 共享**的。别的 session 一起导航，**我的标签页会被带走** ——
而现象恰好是「我改了代码，画面一动不动」，于是很容易误判成缓存、误判成逻辑没生效、
误判成数值不够大，一路往错的方向挖。

这次的实际经过：我在 `planet_x-glsl`(:3004) 上改着色器，浏览器却在
`planet_x-append`(:3005) 上 —— **每一次截图和测量量的都是别人那份代码**。
证据是页面里 `fetch('/shaders/...')` 返回 **404**，而同一路径 `curl` 返回 **200**。

```js
// 每次测量前先跑这个，一行就能定性
const p = await (await fetch('/api/ping')).json();
p.exe      // 必须是你自己的 worktree
```

**代价**：我在这上面白跑了七八轮（自吸收、截断、细步长、uChromo 26→60→5000），
而且每一轮都在向用户解释「这次真的改了」—— 直到量出 404 才发现根本没在看自己的画面。
**多实例并行时，"我以为"是最贵的成本**。
