...
* **一张 36 条候选的「step 中间量」清单已经盘出来了** → 现在**整份落在
  ...
  形状裁决也在那里。
  → **B3 也已经落地**（`feature/b3-market`，见该篇 **§6.4**）：`view.market_trades`（本回合**真的
  成交**的每一笔：价格分解 + 丢货率）、`view.haul_steps`（每艘在跑运输的舰走了哪一步）、
  势力行的购买力/买方名次、逐货栈运力账、禁运三档原因。它顺带验收了本篇形状的两条边界：
  **结算事实 ≠ AI 判定**（前者进新数组 / 新派生表，后者才进 `decisions`）、
  **观测与过程仍然同处一行**、**纯追加 ⇒ digest 逐字不变**（225 绿）。
  → ⚠ **B5 把本篇的两个面重新定义了一次**（`feature/b5-inputs`，见该篇 **§6.6**）：用户裁决
  **`pre` = 输入面**（掷出的随机数 + 判定输入，`RoundInputs`）、**`post` = 结算面**（观测 +
  过程量，`RoundView`），并**砍掉了 `pre` 里那份「回合开始观测」的副本**——实测它与上一回合的
  `post` 逐字段相同（零信息量：同一份 state、同一个 `observe`、空 sink 只把过程量抹成中性值）。
  要读「回合开始时的世界」请读**上一行的 `post`**。所以本篇「pre/post 是同一形状的两份视图」
  这条前提**已作废**：两面现在形状不同、判据是「消费 vs 结算」（而非「时间」），
  文档里凡是按旧义读 `pre` 的注释都按新定义重读。
