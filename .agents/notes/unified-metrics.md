...
- `[x]` 把治理中间量（`governance_total`/`coverage`/距首都距离）也并入 `RoundView`——给
  ...
  **B3（市场与运输）也已落地**（`feature/b3-market`）：第三站——本回合**真的成交**的贸易
  （价格分解 + 丢货率）、每艘在跑运输的舰走了哪一步、购买力与买方名次、逐货栈运力账、
  禁运三档原因；同样是「把 `step_market` / `step_contracts` / `haul_step` 本来就在算的
  东西写进 `RoundSink`」，不是事后重算。见该篇 §6.4。
