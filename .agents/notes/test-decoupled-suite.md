...
搬完之后，**大头从「真跑」变成了「编译 + 投影落盘」**——两者都与「改了多少代码」无关，
而与「要不要付一次冷启动」有关：
...
| 投影 1000 回合（`--index`，169 MB） | **8.6 s** | 纯模拟只要 5.7 s ⇒ **投影多花 2.9 s（34%），全是写 JSON** |
| 投影 400 回合（79 MB） | 5.5 s | ≈ 线性 |
| g3 冷（7 × 1000 回合 + 抽摘要） | 生成 ~60 s CPU（高负载那次 210 s）+ 抽取 ~15 s | `-j 7` 墙钟 ≈ 50 s（I/O 争用：一次写 1.2 GB） |
