...
- **材质的参数 schema 走的是同一本账**：`Material.params` 那袋值（`px_protocol::scene::Value`）只是**载体**，参数名 / 类型 / 字节偏移是渲染器从产物里那份 WGSL 反射出来的 ⇒ 现状、硬边界与「要不要挪进 protocol」的五条候选形态见 `08-renderer.md` **§66**。
