...
- ⚠️⚠️ **上面这条现在被推翻了（用户 2026 裁决）**：**那不是不变式，只是当时不需要**。云的**外接代理 mesh**（§51.10）要求 marching cubes，而 marching cubes 需要一个 **3D 标量网格**；这个网格是 **PCG 的中间值，它和网格（mesh）都要进 PCG 缓存**（键 = 内容，§17.1）—— 这样只改等值面 / 外扩量 / MC 参数时能复用同一张网格，只改网格参数时能复用同一份 mesh。
  为什么当初的顾虑在这里不成立：烘出来的 3D 网格**不参与着色**，只用来把 raymarching 的取值范围收窄（保守外扩 ⇒ 宁可包大），精度不足由"外扩量"吸收，不存在"丢解析、后续处理精度掉"的问题。
  ⇒ §24.3 / §24.4 / §24.5 里关于 **Grid 归属**与 **`Volume3D` 进协议**的部分**重新打开**；cube atlas 布局不变。
