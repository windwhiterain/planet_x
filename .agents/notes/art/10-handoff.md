...
> （2026-09-15/16 那几轮）。现状出口从 `art/15-render-wgpu.md` 起就是**那一篇**（§100–§156，
> 整个 S 系列的过程与判据都在那里）。**正文与全部读数一字未删** —— 它们是那几轮唯一的记录。
> ⚠ 三条会读错的地方，先看这里：
> 1. 正文里几十处 `px_render`（Bevy 宿主）**已经删了**（`f121ee3`，`15-render-wgpu.md` §154）
...
**用户裁决：跳过阶段 0，直接做阶段 1。** 开工序是 **`art/15-render-wgpu.md` §100–§107** ——
