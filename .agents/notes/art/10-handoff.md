...
### 9.1.5 同一天接着的一条**用户报的缺陷**：窗口里的「正中心接缝」——**已定位、已修、已验证**

**用户口径**：**"背面光照会出现跳变"**（2026-09-15，配窗口截图）。全过程与证据链在 `06-clouds.md` **§64**。
...
**修法**：`sun_light` **不问网格**，直接取场景那盏灯（`clustered_lights.data[0]`；点光源住在聚类缓冲里、
Bevy 把点光源排在前面 ⇒ 只摆一盏灯的渲染器，"最近的一盏"就是这一盏）。兜底（没有点光源才退方向光）
**改用颜色判"这格写没写过"**。
⚠ **不能拿 `position_radius.w` 当"有没有灯"的阈值**：那格不是 range（实测对这盏灯读到 0），
拿它判会把有点光源的场景也推进兜底 —— 上一轮那次"整幅受光变了 41.6%"就是这么来的。

**验证（四条单变量）**：① 复现视角那条台阶 **+1.863 → +0.014**；② 出图路四张判据图**逐字节不变**
（`5839AECB… / 11B0A659… / 04FE9DAC…`）⇒ 在原来看得到灯的地方零影响；③ 窗口**初始轨道**修前修后
**除 OSD 外 0 像素**变；④ `cargo test -p px_render` 全绿。⇒ **§59/§60/§63 的图与判据全部保持有效。**

**顺手落仓库的一条**：出图路那盏遗留的 `DirectionalLight { illuminance: 9000.0 }`（`main.rs` 两处）
**已删** —— 删完判据图逐字节不变 ⇒ 它在出图路上从来没生效过，两条路的灯清单从此一致。

**新加的窗口相机 API（复现视角用）**：`px_render --where` 读回当前方位（并打印一行可直接粘回命令行的
`--place yaw,pitch,distance`），`px_render --place y,p,d` 把窗口摆过去（不换场景、不重烘）。
请求/回话走 `target/viewer-scene.json` 的 `ask_camera`/`set_camera` 与 `target/viewer-camera.json`。
...
工具：`target/abstep.py`（大气贡献上最大的台阶）、`target/seam.py`（最长连续跳变段）。

**没做的**：多灯场景仍不支持（取第 0 盏点光源；真正的多灯累加/灯间遮挡见 §60.5）。
