...
### 9.1.5 同一天接着的一条**用户报的缺陷**（缝真实、四条嫌疑已排除、**根因未定**）

**用户口径**：**"背面光照会出现跳变"**（2026-09-15，配窗口截图）。全在 `06-clouds.md` **§64**。

**结论一句话**：窗口里那条**贯穿圆盘的象限接缝真实存在**（用户图 row 572 = 414 px 连续直线 / 31 级；
我按当时相机存的图 row 699 = 138 px / +31.5），**但依赖相机** —— 窗口默认相机下同一套量法**量不到**。

**已排除（单变量实测，全部证伪）**：① §63 的两刀（`orbit-soft-shell` 逐字节等于改前内容）；
② shadow map（`shadows = 0` 不变）；③ 覆盖度 cubemap（`ablate = "fetch"` 照样有）；
④ 大气那条**屏幕空间深度夹取**（换成行星球解析求交 ⇒ 缝一字不变）；
⑤ **聚类光查询返回错的灯**（探针：真实 chord 的 5 个采样点 × 四象限 99.3–99.8% 都是那盏灯）；
⑥ **尺寸错位**（Rust 探针：窗口物理 ＝ `camera.physical_viewport_size` ＝ View uniform `viewport`
＝ 深度图宽 ＝ 片元空间，全是 2240×1400）。
⚠ `--serve` 出图路在评审相机下**干净且逐字节可复现** ⇒ 缝是窗口路独有。

**最重要的一课（方法论）**：窗口的轨道相机**是用户拖的**，我三次"换了缝就没了"的读数
（常数方向 / `light.wgsl` 取 `data[0]` / 之后的受控 A/B）**可能都是在相机漂移之后量的** ⇒ **不算数**。
以后的窗口量法固定为：**重启窗口 ⇒ 连拍 A、B、A 三张**，自检第 1 与第 3 张**除左上 OSD 外逐像素相同**
（实测只差 OSD 区 1209 px、其余 0），才认 A/B。

**顺手做掉的一条（已落仓库）**：出图路那盏遗留的 `DirectionalLight { illuminance: 9000.0 }`
（`main.rs` 两处）**已删** —— 删完出图路四张判据图**逐字节不变** ⇒ 它在出图路上**从来没生效过**
（兜底分支没触发）⇒ §59/§60/§63 的图**不用重出**，两条路的灯清单也一致了。

**没做完 / 待办**：

1. **缝没有确定性复现**，所以还没法二分：建议给窗口加个**临时钩子**（CLI/env 设 `Orbit {yaw,pitch,distance}`，
   现在只有鼠标能改），扫视角找出有缝的那个，再用三拍协议逐行二分 `atmosphere.wgsl`。
   或者扫 `--serve` 的 12 个评审相机（那条路可重跑，但评审相机 1 干净）。
2. 三张临时诊断档（`diag-normals` / `diag-normals-fetch` / `diag-noatmo`）与两种探针**配方在 §64.7**，文件已删。
3. 新仪器 `target/seam.py`、`target/atmodiff.py`、`target/patch.py`、`target/crop.py` 与 `pixdiff.py`
   同放 `target/`，不进版本库。
4. ⚠ `px_render/assets/shaders/{common,light,noise}.wgsl` 是**热载、不在内容键里**（§64.4 第 5 条）：
   改库会静默改所有材质且不使缓存失效 —— 要么并进键，要么写死"改库 = 手动全量重验"。
