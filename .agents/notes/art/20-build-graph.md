...
> 单态化 interface，和普通 graph 以两个 stage 顺序执行」**。§ 号接 `19-generic-inst.md`（`px_inst!`）。
...
> （它不进任何实现库名册 ⇒ 不换键）。设计全文与读数见 **`21-codegen-types.md`**。
...
* 文档代理清了 10 份文件（只碰 `*.md`）：`docs/…integration.md` §5.3 重写成真形状（`px_inst!` + `build()`
  ...
  **§179.5 整节**加过期标记、§179.1 例子改成最终宏签名；`18` §172 加标记（被删的"生成式单态化"今天以
  ...
  （那 31 处历史命令原文一字未改 —— 记录是记录），`art-framework.md` 三处就地改。
...
* `docs/generic-op-and-graph-integration.md` 新增 **§5.4「场域的泛型实例 —— 与体积域逐处同构（能跑的最小例子）」**
