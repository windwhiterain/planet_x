...
| **面板吃掉大半画面** | 960×640 的窗口里，面板约占一半，画面只剩一条 | 面板宽度按**逻辑点**算，而窗口只有 571 个逻辑点宽（960 物理像素 ÷ 缩放 1.75） | `panel.rs::ui` 把上限压成"可用宽度的 35%（≤520 pt）、缺省 30%（≤360 pt）"；`Viewer::size` 仍是物理像素 |
...
### 修好凭据之后**仍然**在的那一条（未解，已上报用户）

交换链那一张的 **x=348..349**（2 像素宽、贯穿 640 行、比左右暗约一半）与离线渲染图不同，
而**离线那张与窗口的 `--shot` 都是 `63184151909371A5`**（都不含这条）。
⇒ 它是**呈现那一段**（`Present::draw` / 交换链）或**回读那一段**的性质，不是场景里的东西。
⚠ 与"读回时机"无关的判据：`--ui-shot` 的读取发生在 `frame.present()` **之前**，
   读的是**已经画完**的那张纹理（同一帧、同一队列顺序），不是"下一帧的未初始化内容"。
⚠ 已排除：面板画在交换链上（`Present::draw` 在全屏三角里盖满全幅、没有 viewport 偏移），
   3D 区其余像素与离线**逐像素相同**。
