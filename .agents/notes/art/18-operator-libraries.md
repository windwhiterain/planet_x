...
> § 号接 `17-typed-ops.md`（§161–§167 是「类型化算子契约」那一轮）。
...
> `20-build-graph.md` §188）。§171.5 那几行**留着当历史值批次**（§六 那一格要拿它们讲事）。
...
> 见 `21-codegen-types.md`；更早几轮是 `px_inst!` 宏，见 `19-generic-inst.md` §174–§180）的形式**回来了**
...
* **两处 `px_cook` 时代的遗留字符串不许顺手改**（它们读起来像可改的，其实是**域分隔符**，
  ...
  两处都已就地写明。
* **画布（`Grid`）仍然只从 `Cache::grid()` 一个出口**：键里那一份与算子手里那一份必然同值。
  这条是上一轮改的，这一轮没动它 —— 记在这里是因为"两处真相"那一类缺陷反复出现过。
