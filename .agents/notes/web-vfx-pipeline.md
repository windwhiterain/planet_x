...
### 补充：日缘那道"缝"的真正成因是**量纲**，不是参数

上一版我把色球的贡献折进日冕的**同一个密度场**、共用 `uIntensity`（0.055）。但物理上
**色球比日冕亮约 10⁴ 倍** —— 共用一个强度，日缘就永远填不满。实测：

```
半径 238→286（每 4px，背景暗侧）
  原始              190 185 191 192 187 | 156 141 140 140 135 129 122 115   ← 缝底 141
  只调参数          198 206 204 191 200 | 175 143 142 141 139 136 132 125   ← 缝底 143，几乎没动
  色球拆出独立强度   201 210 206 197 202 | 183 216 200 189 199 220 230 229   ← 缝没了，成了一圈亮边
```

**改参数没用，改量纲才有用。** 现在 `coronaDensity()` 返回 `vec2(日冕, 色球)`：共用同一套
局部基与噪声（一次算完），但**各自乘自己的强度**（`uIntensity` 0.055 / `uChromo` 26）。

⚠ 顺带一条方法论：**这次「改了却看不到变化」不是缓存问题**。我先去核了
`cache-control: no-cache` 与「服务上内容 == 磁盘」，两者都正常 —— 排除了假象之后，
才敢认定「是改动本身不够」。分文件 fetch 的着色器最容易被误判成缓存问题，
所以这个排除步骤值得每次做。
