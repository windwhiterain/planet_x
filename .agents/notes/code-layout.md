...
| 子模块 | 装什么 | 行数 |
| --- | --- | --- |
| `mod.rs` | `advance`（**回合编排**：生产→承包→维护→市场→建造→军事→治理→迁都→外交→均势→事件跃迁→剧情→思潮→历史裁剪）+ `view_from_state` + 子模块声明与 `pub use` 兜路径 | 170 |
...
| `metrics.rs` | `balance_picture` / `observe`（单一权威观测，当时叫 `round_metrics`） | 128 |
...
* **脚本搬运 + 逐字节自校验**：本次用过的脚本留在 [`scripts/refactor/`](../../scripts/refactor/)
  （带 README：每个脚本改哪里、三个踩过的坑）。做法——把「块拼接回去 == 原文
