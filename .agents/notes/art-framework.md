...
现在两条路径都在 `altitude` 出壳或 `cover <= 0` 时直接返回 0 ✓，与 FD 版一致 ✓。

⚠ **下面这段是错的，§45.2 纠正**：「FD 版本在壳边界会返回巨大的差商，而 `ABLATE_NORMALS`
正是靠这个尖峰找表面的」✗ —— 找面靠的是对 `cloud_field` 的步进
（`if cloud_field(point) > SURFACE_LEVEL { break }`），**梯度只用来算法线** ✓。
所以换解析版不改变找面的行为；硬表面云**已经换掉了** ✓。
...
## 44. 梯度验证的三条腿，各自覆盖什么（别再搞混）

写解析梯度之前必须先有基准。基准一共**三条腿**，它们**覆盖不同的失效模式，缺一条都不够**：
...
| **③ WGSL 逐点对拍**（`px_render/tests/field_dual.rs`） | **发布的 WGSL 里**，解析梯度 = 值函数的导数吗？ | **代数风险（全式）＋ 它就在正确的边界一侧** | ✓ | ✓ |
| **④ 场级 f64 仲裁者**（`px_verify/src/cloud_field.rs`） | **基准本身**对不对？ | **基准的失效**（差商底、相消、折点） | ✗ | ✓ |

⚠ **①②都只在"Rust 函数"这一侧** —— 它们**回答不了"Rust 的函数 = WGSL 的函数吗"** ✗。
那条边界才是真正的风险所在。

⚠⚠ **但 ③ 也不够 —— 这是 §44 原表漏掉的一格**：③ 拿差商当基准，而差商在生产配置上
**自己就有 6.6e-4 ~ 1.1e-2 的底**（§45.2），故障量级 1.9e-1 只比它大 30 倍 ⇒
**③ 单独分不清"公式错"和"基准不够格"** ✗。**④ 因此是第一等的腿，不是兜底** ✓。

### 44.1 顺序被用户纠正过一次，记下来

我原来的计划是**先**把 `shape_of` 移植到 Rust 建场级参考，再对拍。
用户问"1 和 2 都在验证 rust 函数，是否重复" —— **问题问对了**：那个计划把**第三份实现**当成前提 ✗，
而它还在边界的**错误一侧**。

**改后的顺序** ✓：**先建 ③（GPU 上的 WGSL 自对拍）**，
Rust 的场级 f64 参考**降级为"仲裁者"** —— 只在 ③ 结果模棱两可（比如折点附近）时出手。
⇒ **少一份必须维护的重复实现** ✓。

⚠ **这一条被实测推翻了，见 §45.2** ✗：③ 拿差商当基准，而差商在生产配置上的底
（6.6e-4 → 1.1e-2，越细越差）和故障量级（1.9e-1）只差 30 倍 ⇒ 不是"模棱两可"，
是**基准不够格**。降级仲裁者的代价是：**唯一能判定的仪器被降级掉了** ✗。
⇒ ④ 必须是第一等的腿 ✓。

### 44.2 判据：扫 h，不要拍容忍度

**这一条已经用一次就回本了** ✓。噪声对拍第一次跑出 7% 偏差，看着像公式错：
...
**教科书 U 形**：左截断、右相消、真值在中间。**对偶数一直是对的，是我用截断误差 7% 的步长在量它。**
...
`clamp` / `max` / `floor` 边界处导数**不存在**。对偶数返回**它实际走到那一支的单侧导数** ✓；
中心差分**跨过折点**，给的是另一个东西 ✗。**两者在折点邻域不可比。**

⚠ 而且**按"平均值"过滤抓不到它**：`fbm_3` 对**三个各自 `clamp01` 的八度取平均**，
**内层八度饱和时平均值看着完全正常** ✓ —— 这是我踩过的坑，诊断打印才看见某八度正好是 `1.000000000`。

### 44.4 headless wgpu 的地基（已验）

`px_render` 是 **binary crate，测试 `use` 不到它的任何类型** ✗ —— 报告点名的结构性缺口。
已决定**加 lib target**（用户拍板 A，不选"测试里镜像一份"）。
无窗口设备实测：`Instance::default()` + `request_adapter(compatible_surface: None)`，
**4.24 秒**拿到 RTX 3060 —— ⚠ 注意它走的是 **Vulkan 不是 DX12**（`Instance::default()` 自己挑后端），
以后对拍出现说不清的差异时要记得这个变量。

---

## 45. 精确仲裁者建起来了，它推翻了 §44 的顺序结论（2026-09-14）

### 45.1 形状：独立 crate `px_verify`
...
`px_render` 以 **dev-dependency** 引用它 ✓；`px_ops` **不引** ✓
（`Scalar` trait 留在 `px_ops` —— 它只是个 trait，不是脚手架）。

⚠ **对偶数给的是精确导数** ⇒ **不需要步长、不需要折点余量、不需要扫 h、不需要退到简化配置** ✓✓。
§44.2 那一整套扫 h 的机械，存在的唯一理由是**基准是差分**。基准精确了，它们全部消失。

### 45.2 三处纠正（§43–44 里已经写错的地方）

1. **§43.1「`ABLATE_NORMALS` 靠 FD 的尖峰找表面」是错的** ✓。找面靠的是
   `if cloud_field(point) > SURFACE_LEVEL { break }`，**梯度只喂法线** ✓。
   硬表面云**已经换成 `cloud_field_gradient_analytic`** ✓，FD 版 `cloud_field_gradient`
   和只服务于它的 `SURFACE_EPSILON` 一起删掉 ✓（每像素少 6 次 `cloud_field` 调用）。

2. **§44.1 的顺序结论被实测推翻** ✓。原结论："③ 先建，Rust 场级参考**降级为仲裁者**，
   只在模棱两可时出手 ⇒ 少一份重复实现"。
   实测：③ 拿差商当基准，而差商在**生产配置**上的底是

   ```
   h=8e-5  6.6e-4      ← 最好的
   h=5e-6  1.1e-2      ← 越细越差（f32 相消占了上风）
   ```
   故障量级是 **1.9e-1** —— 只比底大 **30 倍**。这不是"模棱两可"，是**基准不够格** ✗。
   ⇒ **仲裁者必须是第一等的腿** ✓；把它降级掉的代价是**唯一能判定的仪器没了** ✓。

3. **§43.2 的「做 dev-dependency」作废** ✓ —— 用户否掉了塞进 `px_ops`：
   脚手架不该长在出图的 PCG crate 上，于是有 `px_verify` ✓。

### 45.3 找到的东西

**(a) `px_ops` 的噪声和 shader 的噪声不是同一个函数** ✓✓ —— **生产问题，待裁决**。
...
固定点 `[1.3, 2.7, -0.4]`、seed 7：Rust **0.33985612** 对 shader **0.31646734** ✓。
变量名叫 `tx`，所以这个替换在 Rust 里**看不见** —— `weight` 用对了，`dot` 用错了 ✓。
...
`baked.g` 是 `∂mask/∂x`，是梯度的**分量**，不可能同时当**标量因子** ✓。

**(c) f16 覆盖度贴图当不了这个 oracle** ✓ —— mask 在 4h 上只走 **5.7e-5**，
而 f16 在 0.5 附近的台阶是 **2.44e-4**（大 4.3 倍），过滤后成阶梯，**差商恒为零** ✓✓。
**这才是盲区"为什么"存在** ✓，而不只是"存在" ✓。
（⚠ 生产烘那张图用 `Rgba16Float` **本身够用** ✓ —— 它存的是**值**不是**差商** ✓；
只有拿它当差分 oracle 才会坏 ✓。）

### 45.4 两侧对照不是形式主义（我自己踩过一次）

arbiter 一度报「**修复前** 3.1e-3 反而比**修复后** 2.0e-2 **更接近**精确值」✗
—— 即"你的修复把东西改坏了"。

原因：我把 `∇mask` 当成 `∇cover` 送进模型，**少乘了 chain** ✓。于是参考值落在 **1×**，
修复后 **4.9×**，修复前 **0.54×** —— 1× 自然离 0.54× 更近 ✓。

⇒ **单向断言（"偏差 < 阈值"）在这里会判反，而且正好放过真 bug** ✓✓。
现在覆盖度那条是**两侧**断言的：修复后 **4.288e-6**，修复前 **2.348e-2**，**相差 5476 倍** ✓。
...
- **别拿差商当"公式对不对"的判官** —— 先量**基准自己的底** ✓。
  差商的底 ≈ `ε/(2h) × 场值量级 / 梯度量级`；壳只有 0.05 厚、|∇ρ| ~ 500 时，
  h=5e-6 的底就有 **1e-2** ✓。
- **两侧断言**：只测"新的对不对"会漏掉"参考实现自己写错了"，而且会**判反** ✓（§45.4）。
- **子 agent 会被打断，且可续跑** ✓：先 `list_agents` 看状态，`[ready]` 可用 `send_message`
  续跑；被打断 ≠ 失败，**不要新派一个重做** ✓。
