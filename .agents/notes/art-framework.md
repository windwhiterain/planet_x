...
2. **就绪判据不能把"失败"当"完成"** ✗✓。旧 `watch_pipelines` 里 `Err` 既不算 pending 也不算失败
   ...
   **症状特征**：一整轮参数扫描的 PNG **字节数完全相同** ✗（我第一次就撞上了，
   ...
   一行就定位了上面那条 ✓。
3. **反复热重载同一个材质 shader 会让服务劣化，而且不恢复** ✗。
   实测：同一进程内改 `clouds.wgsl` 三次之后，**连"无云"场景**也从 383 ms 涨到 6771 ms ✗；
   `--serve` 重启后立刻回到 396 ms ✓。另有一次直接 **DX12 DeviceLost**
   （`GraphicsCommandList::close failed 0x80004005`，随后 `DeviceLost` 退出）✗。
   ⇒ **热重载适合"改一次看一眼"** ✓；要连改十次，就批量改完重启服务再测 ✓。
   §38.2 那句"改 shader 不重建不重启"当年只在**大气** shader 上验过一次，**不能外推** ✗。
