...
`engine_hash` = **正在执行的那支二进制**的代码指纹：`px_ops` 与 `px_graphs` 的全部源码 +
`Cargo.lock` + rustc 版本（`build.rs` 编译期算好、`env!` 进二进制）。

- **不需要 `op_version`**：改算子代码 ⇒ `engine_hash` 变 ⇒ 整个缓存自动失效。
- **不需要门脚本**：没有需要人去维护的东西。
- 参数迭代（热路径）不动二进制 ⇒ `engine_hash` 不变 ⇒ **缓存照常命中**。
  ⇒ 恰好是我们要的语义：**改参数不失效、改代码才失效。**

⚠️ **我补的洞：`engine_hash` 必须同时覆盖"图程序自己的代码"，不能只覆盖 `px_ops`。**
因为拓扑在 Rust 里，一句 `field::mix(&a, &b, 0.35)` 里的 `0.35` 是**写在代码里的常量**，
它不进 `.pxp`、也不改任何输入的 key ⇒ 光靠 §16.3 那条"结构变化沿 key 链传播"**抓不到它**。
把图程序的代码也纳入 `engine_hash`，这个洞就没了。

**代价（要认）**：**任何一次代码改动都让整个缓存失效**。写 `px_ops` 时会全量重烘。
但改代码是你主动做的、且你知道自己在改代码，所以这个代价是可预期的；
而"旧缓存被错误命中 ⇒ 画面悄悄不对"是不可预期的。**用可预期换不可预期，值。**
（若将来重烘真的疼，细化路径是每算子 `include_str!(file!())` 自哈希——精确到文件、仍然自动；
它的弱点是算子依赖的公共模块改动抓不到，所以现在不做。）
