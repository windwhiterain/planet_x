...
| 腿 | 回答的问题 | 覆盖 | 需要 GPU |
|---|---|---|---|
| **① 对偶数语义**（`px_ops/tests/dual.rs`） | 仪器本身准吗？ | 我手写的 `dmax/dmin/dclamp/dsmoothstep` 对不对 | ✗ |
| **② 噪声层对拍**（`px_ops/tests/dual_noise.rs`） | 噪声的对偶梯度 = 它自己值函数的导数吗？ | **代数风险**（噪声部分） | ✗ |
| **③ WGSL 自对拍**（待建） | **发布的 WGSL 里**，解析梯度 = 值函数的导数吗？ | **代数风险（全式）＋ 它就在正确的边界一侧** | ✓ |

⚠ **①②都只在"Rust 函数"这一侧** —— 它们**回答不了"Rust 的函数 = WGSL 的函数吗"** ✗。
那条边界才是现在真正的风险所在，**所以 ③ 不能省**。
