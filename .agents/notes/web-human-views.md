...
>
> 下一步（用户已定）：**D8 = Rust 结构体字段按重要性重排**（见 §10.6）；写面 spec 化（D5）**已开工**——
> 第一步（引擎发叶的结构事实 + `views.json` 四种行 + 读/控制穿插）落地在
> [`web-control-spec.md`](web-control-spec.md)（分支 `feature/web-control-spec`）。
...
| **D5** | 写面 spec 化 | **分两步**：本轮只做读面组织层（控制树原样保留）→ **第二步已开工**，见 [`web-control-spec.md`](web-control-spec.md)（S1–S5 已裁决：引擎发结构事实、两栏合并、多键叶进卡片、Rust 检查搬 Python、先交第一步） |
...
* **D5：控制树（写面 1200 行）的 spec 化**——**已开工**（[`web-control-spec.md`](web-control-spec.md)）：
  第一步把「叶的结构事实」交给引擎发、`views.json` 支持 `path`/`leaf`/`owner`/`action` 四种行、
  左栏两个模式合并（旧控制树降级成「控制树（旧）」页）；**删旧树是第二步**，还没做。
  ⚠ 本文件 §8 说的「前端没有测试设施 ⇒ 纪律交给 `web/src/views_tests.rs`」已经作废：
  那份 488 行 Rust 检查**搬到了 `play/tests/g4_spec.py`**（用户裁决：本仓测试往 Python 搬）。
