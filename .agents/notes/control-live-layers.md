...
* `[x]` **web 侧还差两行** —— **已补上**（本轮分支 `feature/web-fleet-defaults-style`，实现与
  实机证据见 §7）。剩下的是**投影表的四条风格叶**（不是 web）：`src/projection.rs` 的 `control`
  tidy 表只有 `ship_order` / `default_ship_order` / 预算 / 权重 / 首都，缺
  `ship_doctrine` / `ship_kiting` / `default_doctrine` / `default_kiting`——Python 侧现在只能从
  `ships` 表的 `doctrine`/`kiting`（有效值）看结果，看不到这四片叶自己的值与表态。
  `kind` 的 schema 描述（`projection.rs:662`）也还只列着老的七种。
