...
| **T1** | ≤ 48 回合（4 年内） | `cargo nextest run` | **155 条 / 2.2 s**（另 31 条 `#[ignore]` 探针） |
| **T2** | 49–480 回合（4–40 年） | `cargo nextest run -P mid` | **同一批 186 条**（Rust 侧 T2 已空） |
| **T3** | > 480 回合（40 年+） | `cargo nextest run -P full`（合流门） | **同一批 186 条 / 2.2 s**（Rust 侧 T3 已空） |
...
* 集成测试：**模块**就叫这个名字 → `tests/probes/horizon_long.rs`（用例名 `horizon_long::…`）
  ...
  **不要再写 `binary(horizon_mid)` / `binary(horizon_long)`**（nextest 对匹配不到任何二进制的
  ...
  （155 条断言 + 31 条 `#[ignore]` 探针；2026-10 第 7 批收口时的数）。
