...
| **T1** | ≤ 48 回合（4 年内） | `cargo nextest run` | **178 条 / 2.0 s**（dev 档 4.4 s） |
| **T2** | 49–480 回合（4–40 年） | `cargo nextest run -P mid` | 184 条 / **8.0 s**（dev 档 29.7 s） |
| **T3** | > 480 回合（40 年+） | `cargo nextest run -P full`（合流门） | 214 条 / **未重测**（dev 档 95.8 s；按 4.27× 推算 ~25–30 s） |
