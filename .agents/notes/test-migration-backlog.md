...
### 订正：口径不一致**与玩家钉死无关**，原因待定（2026-10 第 7 批）

把"逐舰机会值"补全之后（两枚骰子**各带自己的 `flow`**——同一艘舰会先后掷观测与运输两枚，
共用一个字段会被后一枚覆盖，这是第一版的错），实测 `Σp == flow`：

```
7 seed × 1000 回合：25158 行 → 等号 22852 行 ✓、Σp < flow 2306 行 ✗、被 min(1,·) 截断 1155 行
```

**我一度把原因归到「订单叶被玩家钉死的舰还在分母里」**——用 `ships.order_leaf_mode` 判时，
"不一致的 103 行全都有被钉死的舰、干净势力 0 行"，看着像铁证。**改用引擎真正读的
`ships.order_effective_mode` 之后那一类变成 0 行** ✗ ⇒ 与玩家钉死**无关**；那次"确认"是
用错字段（**叶**模式 vs 引擎**解析后**的模式）造成的伪证。

**下一步**：把 `pool_n` / `tickets` 也记进 `role_distribution`（池子几个人、票摊了多大），
再挑一行（如 `中国 r2 freight/join`：`Σp = 0` 而 `flow = 0.05`）逐舰对，才能定因。
在那之前 g3 只压**单向界** `Σp ≤ flow`，等号那一半**如实报数、不判红**；两条抽样循环
（`the_observer_headcount_lands_on_the_quota` / `observers_and_haulers_both_get_their_share`）
**暂不删**。

### 结案（同日）：**没有 bug** —— 那 2306 行是我判据的错

上一节的"口径不一致"**不成立**。用正确口径重测（3 seed × 300 回合）：`Σp == flow`
**721/487、837/772、778/586 行全等，违反 0**。之前那 2306 行全部来自**我的对账口径错**：

* 池子里**记到过舰**（`mine_* > 0`）；
* 池子**非空**：`tickets_*` 等于 `1e-9` 是 `max(1e-9)` 的**地板值** ⇒ 池子空。典型例子
  `中国 r2 freight/join`：北斗在 r1 末尾是**观测舰**，按设计被排在自己的运输池外 ⇒ `我的票 = 0`
  ⇒ `p = 0` **合法**；而我却把"为它算出的 `flow`（0.05）"当成了那一侧的总期望 ✗；
* **记下的舰正好是池子**（`Σ我的票 == 同侧总票数`），且没被 `min(1,·)` 截断。

另外我用闸门里带的池子名单**点过名**：池子里**没有一艘"有票却没有闸门"的舰**（0 例）
⇒ 不存在"分母里挂着永远不会被定编的舰"的稀释。引擎注释里记的那次稀释事故（4.30 的配额只跑到
3.65）是**已经修好的历史**，现行实现自洽。

**因此**：`Σp == flow` 现在是**精确判据**（g3 `distribution_checks`，7 seed × 1000 回合
**22863 行逐字对上、0 违反**），替掉了那两条抽样循环；两条循环**已删**（Rust 153 → **151**，
真跑 4.3 → **2.5 s**）。

**这一段的教训**（同一个角落里我错了四次）：索引差一、`order_leaf_mode` vs `order_effective_mode`、
一个字段装两枚骰子、**把"池外舰的 flow"当成总期望**。共同点都是**先信自己的口径、再去解释引擎**。
先问一句"这个数是谁算的、它算的是哪一批舰"，能省掉全部四次。

## 测试时间：统一视野 + 跨组共用轨迹（2026-10 第 7 批末）

用户裁决：*「不重叠你可以让他重叠啊」*、*「只跑短组的时候轨迹只生成到短的，跑长的时候再接上去生成」*。

* **g2 的主世界与 g3 的种子集合并**（g3 用 `(1,2,3,4,5,7,11)`、g2 用 `(1,7,11)`，**同一批 400 回合目录**）；
* **视野 1000 → 400**（按前缀量过：定编对账行 10539、维护费欠费 382 —— 都远高于各判据的防空转下限；
  1000 回合分别是 22863 / 840）。加 seed 42 会引入一条 **MOND 律漏档**的假红（边界上目标都是 1.0、
  中途掉下去 ⇒ 引擎按松弛走是对的）⇒ 那条律要补"回合中途进出带内"的夹逼，**先不动**，种子集改用已有的 11。
* `_harness.run_into` 每次生成都留 `_ckpt.json`；`projection` 加两条近路：
  **截断**（有更长轨迹 ⇒ 拷它的前缀）与**续跑**（有更短轨迹且带存档 ⇒ `--start` 续跑再把前缀拼回来）。
  ⚠ 拼接必须搬**整目录**（顶层 `main.jsonl` / `meta.json` / `idx/schema.json` 一个都不能漏），
  且用 `newline=""` 写（否则 Windows 把 `\n` 翻成 `\r\n`，字节就不等了）。实测 21/21 个文件逐字全等。
* **账**：冷跑 ~6 min → **96 s**；温跑 54 s → **30–60 s**（机器噪声）。缓存 3.3 GB → **~1 GB**。
  Rust 门 ~8.5 s（真跑 2.5 s）已经不是大头 —— **时间现在主要花在"生成/解析轨迹"上，不在断言上**。

### 表复用（同一天稍后）

用户问「每个测试共享 load 的表了吗」——**没有**：g2 一共 `KIT.load` **63 次**、只有 **31 个不同
`(目录, only)` 组合** ⇒ **32 次重复装**（某个世界的 `only=("cities",)` 装了 **8 遍**），装表总耗时
**39.3 s** ≈ g2 的整个墙钟。`_harness` 里加了 `(目录, only)` 记忆化 ⇒ **39.3 → 12.1 s**。
⚠ 帧因此**共享**（各组只读；要就地改先 `.copy()`）。
`run.py -j N` 是**世界级**并行（`--index` 那层），不是组间并行。

**顺带查到一件更要紧的事**：g2 那次 17 条红**全部是"旧二进制"** —— `feature/layered-read` 把
`GameEvent` 载荷字段改名（`shots` → `逐发`，`src/model/event.rs`）已经随合流进来，而我后面几次
量时间都用 `run.py --no-build`，二进制还停在合并前 ⇒ 判据读 `逐发`、引擎吐 `shots` ⇒ 17 条红。
`cargo build --release` 之后 **202/202 绿**。`run.py` 的过期判断扫的是 `("src","Cargo.toml","config")`，
**没有 bug** —— 是"用了 `--no-build` 就得知道自己在干嘛"。教训记在这里：**合流之后先重编再判红**。

另：这台机器当时被**别的 worktree 的 g3**（`planet_x-layers` / `planet_x-ev2`，各 ~2.5 GB）占到
**96% 内存**，OpenBLAS 一度直接分配失败 ⇒ 那天所有墙钟数（30–63 s 抖动）都是在内存压力下量的。
