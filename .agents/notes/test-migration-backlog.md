...
> 计数：**Python 322**（g1 77 / g2 186 / g3 39 / g4 23）；**Rust 155**（+31 探针 ignored）。
...
⚠ **思潮四轴的「朝目标走」没搬**，两种写法都试过：
* **逐回合法条**：列过 `r2`、每回合位移 ≤ 0.05 ⇒ 判据会退化成「怎么都过」；
* **区间夹逼**（下一回合的值必须落在「旧值 ↔ 目标」之间）：把四轴目标全从读面复算
  （军事 = `--call military_deltas` 喂当回合事件、经济 = 价值加权产出 − 维护 − 治理、
  科学 = 异常区城数 − 舰数、自然 = 人均面积），**越界率仍有 2.6%**；把目标错后一回合
  （相位差假设）反而更差（3.9% / 6.0%）。剩下的偏差来自 `step_ideology` 用的是**走那一刻**的
  state 与 flow，而读面只有回合末的值。
