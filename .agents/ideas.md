...
- `[~]` **海军多样/混编（关键下一项）**：诊断（3 种子）显示长局**舰队几乎全是护卫舰**（某
  种子 3000 回合 90-104 艘，全部 `corvette`，无驱逐/巡洋/战列/航母）——基线 AI 在军备收敛
  预算下只造最便宜的舰，导致海军单一、缺乏轻重搭配。根因：`choose_next_class` 随机挑可负担
  舰级 + 建造预算偏保守（`upkeep_reserve_mult` 按 4×维护费预留），便宜的护卫舰先走完
  `build_points`，贵的舰级进度填不满。目标是让 AI **按资源/威胁混编**：主力舰科技/资源够了就
  多造巡洋/战列/航母，缺钱就护卫+驱逐；可用一个「目标兵力构成」加权来让 `choose_next_class`
  和船坞 `ship_type` 偏向补齐短板。建议先用 probe 抓「各舰级产量/在役数」时序再调。

- `[ ]` **AI 军事智能（更拟人，续）**：撤退/集火已做。剩余：**反导弹特化**（有 PD 的舰
  优先拦导弹、有导弹的舰优先打无 PD 目标）、**护航**（保护航母/运输舰这类高价值舰种）、
  以及造舰的**威胁响应**（邻敌多时就多造剑/护盾，否则多造殖民/经济舰）。全部确定性。
