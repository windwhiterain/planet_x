...
- `[x]` **拟人战斗 AI：自保撤退 + 集中火力**（已实现，`sim.rs` 的 `prefer_target`/
  ...
  * 测试：`combat_concentrates_fire_on_wounded_enemy`、`damaged_far_ai_ship_withdraws_to_heal`；
    ...
    种子 1/12345 在 3000 回合仍 71-115 舰、价值 709-1073（健康）；seed 42 偏稀疏（8 舰/61 价值）。
- `[ ]` **基线 AI 军事经济平衡（关键下一项）**：诊断显示**自动基线 AI 长局会军备/经济「降温」**
  ——早期大建舰队（seed 42 一度 143 舰）→ 维护开销 + 军备损耗拖垮经济（世界价值从数千降到
  数十）→ 稳态舰数降到个位数。根因是**基线 AI 造舰不按可持续性**（无脑大建、不随可负担度
  收敛），且**打残后不优先重建**、**战争期间不主动再殖民**（城市在长局里会掉到 9，全靠
  resurgence 兜底）。目标是让基线 AI 更拟人：造舰预算**按可负担度**收敛、战损后**优先补舰**、
  和平/战时不疏于**再殖民**（保持城市/生产规模），才能让上千回合世界既活跃又不崩坏。
  建议先用 `probe` 类观测把「城市数 / 舰数 / 世界价值」随回合的时序抓出来，再据此调
  `construction_budget` 的默认值与冒泡策略。

- `[ ]` **AI 军事智能（更拟人，续）**：撤退/集火已做。剩余：**反导弹特化**（有 PD 的舰
  优先拦导弹、有导弹的舰优先打无 PD 目标）、**护航**（保护航母/运输舰这类高价值舰种）、
  以及**造舰按威胁响应**（邻敌多时就多造剑/护盾，否则多造殖民/经济舰）。全部确定性。
- `[ ]` **武器/护盾失衡观测与调参**：`choose_loadout` 目前偏「挑分最高、买得起」，长局
  里某势力可能过度堆同一组件。可加**类别配比约束**（如武器:防御 ≈ 2:1、每舰至少一件防御），
  避免舰队全是玻璃大炮或全是乌龟。先跑 `probe` 类观测量化再动手。
...
- `[ ]` **护盾对物理/能量防御的差异深挖**：已有 shield_mult/hull_mult，可再细分
  ...
  取舍。
- `[ ]` **损坏/战术损伤**：当前舰的「损毁」只有 hull≤0 一途。可加「模块损毁」（武器/
  护盾被打掉后该舰变弱），让战斗有「逐步丧失战力」的渐进感（长局更真实、不太过一触发即
  碾压）。注意保持确定性。
