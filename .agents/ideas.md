...
`sanction_trade_mult`）。带来 `CoalitionFormed`/`CoalitionEnded` 事件，并把当前格局
（`hegemon`/`members`/`power_share`）暴露给 agent 的 `coalition` 字段与 `meta` 的
...
- `[ ]` **霸权疲劳 / 厌战螺旋**：当某势力控制定居点占比超过阈值（如 55%）时，国内士气/
  忠诚/生产走低，产出一个「高处不胜寒」的惩罚，逼其体量回落到稳定区间。当前一版靠
  「制衡 + 治理超载」间接实现，但**仍然允许某势力在长局里长期占 ~55% 城市份额**（弱
  势力虽抱团制裁，却不温不火）。这是下一步主攻：需要让抱团真的**咬下去**（见下文）。
- `[ ]` **多极硬指标强化**：`world_is_multipolar` 目前只断言「不统一 (<0.85) + 联盟出现过」。
  更硬的模式可断言「任意一方城市占比 < 阈值」或「霸权会轮换（长局出现 ≥2 个不同最强
  势力）」，但目前 seed 42 会长期锁定同一最强势力，故暂未强加——先把「让抱团真的咬
  下去」做好。
- `[ ]` **抱团的牙齿 / 联盟舰队协同**：当前弱势力虽结成反制联盟，但各舰仍按「就近敌人」
  独立行动，**没有真正的联军合力**——所以联盟常打不赢霸权，反而让霸权先吞并弱国。可给
  联盟加一个**协同倾向**：当某方是霸权时，结盟势力的舰优先集结、优先攻击该霸权的
  军力/城市，形成「集体制衡」。
- `[ ]` **经济制裁落到「扩张能力」**：当前制裁只缩水自动市场交易额度，对能自给自足的
  资源大国（如星系矿业/深空运输）收效甚微，无法阻止它们闲庭信步地殖民、长期占 ~55%
  份额。可让制裁进一步**抬高其治理/维持成本**（被封锁的霸权殖民地更难养、远程舰队
  维护更贵），把「多国资源封锁」真正转化为「霸权扩张受限」。
