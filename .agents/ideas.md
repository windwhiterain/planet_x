...
- `[x]` **`RoundMetrics` / `FactionMetrics`**（`src/model.rs`，均 `#[derive(..., JsonSchema)]`）：
  一回合的总结指标类型。字段：世界级 `cities/ships/fleet_value/population`、`power_share`
  （各势力实力占比）、`hegemon`、`coalition_members`、`sanctioned`、`wars`（交战对，无序），
  以及 `factions`（key=faction id → `FactionMetrics{city_count,ship_count,fleet_value,
  population,market_value,at_war}`）。
- `[x]` **`sim::round_metrics(state, config)`**（`src/sim.rs`）：**唯一权威**的总结聚合器，
...
- `[x]` **agent 视图带 `metrics`**（`src/agent.rs`）：`Trajectory` 增加 `pub metrics: RoundMetrics`；
  `state_json`/`render_state` 签名改为带 `&GameConfig`，内部调 `sim::round_metrics`。因此 agent
  在每个回合 JSON 里**同时看到直接状态 + 总结**，且 schema 自描述（`agent_view_is_self_described_by_schema`
  增加对 `metrics` 的断言）。
- `[x]` **`--digest` 与逐回合视图同源**（`src/main.rs`）：`run_digest` 在窗口末态调用
  `sim::round_metrics`，`digest_value` 直接读它（删除重复的 `active_war_pairs` 与独立的
  势力/世界聚合逻辑）；digest 也随之**新增** `population/market_value/at_war/sanctioned` 字段。
...
- 验证：`cargo build` 无 warning；`cargo test --lib` 39 passed；`cargo test --test longhorizon`
  5 passed / 4 ignored。smoke（`--seed 7`）：`--round 12 --every 3` 每行带 `metrics`
  （cities/ships/fleet_value/hegemon/wars/factions…）；`--traj 6` 的每个 `trajectory[i]`
  带 `metrics`；`--digest 12` 输出含 `power_share/hegemon/coalition_members/sanctioned/wars/
  events/story`，且不再含 `-0.0`。
