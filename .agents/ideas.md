...
- `[~]` **AI 军事智能（更拟人，续）**：撤退/集火/混编/武器克制/威胁响应造舰已做。剩余：
  **护航**（保护航母/运输舰这类高价值舰种）、「有 PD 的舰队优先拦截来袭导弹流」（当前 PD
  是按距离择敌开火，可加显式拦截逻辑）、以及**整支舰队随威胁重构**（把既有船坞的
  `ship_type` 按战况重定向——让`威胁响应`不只作用于新建舰厂）。全部确定性。
