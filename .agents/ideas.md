...
## 18. Agent 游玩体验优化（游玩摩擦点） — `[~]`（见 §26：闭环视角已做，语义指令助手仍未做）
...
- `[x]` **策略护栏入文档**（**已做，见 §26.2**）：`.agents/agent-play.md` §6.1「红线是量出来的」
...
## 26. Agent 游玩体验打磨（真玩一局 → 修掉「静默失败」与文档腐烂） — `[x]`

> 动机：§18 把「agent 玩起来难受在哪」列成了清单，但那一轮是**读代码猜**的。这一轮的做法是
...
- `[x]` **① 手册教的控制面早已被移除（最贵的一条）**：`agent-play.md` §4.2 的招牌范例写
  `{"type":"target_ship","ship":"华盛顿","attack":true}`，而 §21 的**行为重构**早已删掉
...
- `[x]` **§6.1「红线是量出来的」**（把 §18.6 的「量化阈值」补上，用本轮**真实游玩数据**）：
...
- `[ ]` **§18.1 的语义指令助手**（`--fight`/`--build`/`--hold`/`--colonize`/`--loyalty`）——
...
- `[ ]` **模板瘦身 / `--control <faction>` 收敛**（§18.3 的 `--focus`）：`--control` 整面
...
- `[ ]` **（游戏侧，非体验侧）平衡观察**：官方 AI 的 `中国` 会**反复**走「舰队送死 → 造养不起的
  舰 → 被夷平」这条路（两种 seed 都一样），且 §23 遗留的 `upkeep_shortfall` 报废与这里同源。
