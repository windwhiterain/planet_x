...
agent 每舰暴露 components 与 effective 面板，meta 暴露组件表。测试：4 个新单测（面板/
选装确定性/护盾+规避/长局造舰定制）；5 个长局守卫全过；3 种子 × 3000 回合诊断无 NaN、无
panic，debug 约 75s（与此前量级相当，战斗 O(舰×武器) 摊薄，主要开销仍是既有 O(n²) 找目标）。

- `[ ]` **AI 军事智能（更拟人）**：舰船自动行为目前只有「就近开火/默认 resolve_target」。
  可加 撤退（受损舰回本土修整/充盾）、集中兵力（围点打援，优先集火威胁最大的敌舰组而非
  最近那艘）、反导弹特化（有 PD 的舰优先拦导弹、有导弹的舰优先打无 PD 目标）、以及
  「造舰按威胁响应」（邻敌多时就多造剑/护盾，否则多造殖民/经济舰）。这些能让 AI 的军事
  决策更像人，且全部确定性。
