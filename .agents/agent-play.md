...
`--index` 投影里，每回合一行 **lean 主流**（`main.jsonl`），eager 字段：

```jsonc
{ "round": 12, "time_month": 12.0,
  "events": [...], "chronicle": [...],
  "metrics": { "cities": 17, "ships": 26, "fleet_value": 578.0, "population": 8800,
               "power_share": {"3": 0.33, ...}, "hegemon": 3, "sanctioned": 3,
               "coalition_members": [...], "wars": [[1,3], ...],
               "factions": {"3": {"city_count":4,"ship_count":6,"fleet_value":120,
                                   "production_value":64.8,"upkeep":30.0,
                                   "governance_cost":4.8,"at_war":true}, ...},
               "city_production": {"0": {"production_value":28.4, ...}} },
  "ship_ids": [0,1,2, ...], "city_ids": [0,1, ...], "body_ids": [0,1, ...] }
```

重型实体**不内联**，只在 `idx/*.jsonl` 表里、按 id 索引：

| 表 | 粒度 | 关键列 |
|---|---|---|
| `ships` | 每回合 | `ship_id faction_id class hull hull_max shield shield_max x y velocity` |
| `cities` | 每回合 | `city_id faction_id body_id name population loyalty razed building_count deployed_area` |
...
q.ships(round=12)             # 第 12 月全部舰
q.cities(round=12)            # 第 12 月全部城
q.bodies()                    # 天体主表
q.ids("ships", 12)            # 第 12 月的 ship_id 数组
...
# 我（faction id=3）的走势
f3 = q.facts.metrics.apply(lambda m: m["factions"].get("3", {}))
f3.apply(lambda d: (d["city_count"], d["ship_count"], round(d["production_value"],1)))

# 边缘失稳城（忠诚低，易叛乱）
q.join("cities", round=12).query("faction_id==3 and loyalty < 0.5")
```
...
# 想把观察面/控制面收敛到单个势力 → 见 §18.3 的 --focus / --view 建议（尚未实现）。
```

### 4.2 diff 实例（中国 id=3：造护卫舰、守地球、稳住忠诚——**局部接管**）
```jsonc
{
  "control": [{
    "faction_id": 3,
    ...
    "loyalty_budget": [ {"city": 0, "value": 2.0, "mode": "Player"} ],
    "ship_orders": [
      {"ship": 0, "behavior": {"type": "target_ship", "ship": 6, "attack": true},  "mode": "Player"},
      {"ship": 1, "behavior": {"type": "target_ship", "ship": 2, "attack": false}, "mode": "Player"}
    ]
  }]
}
```
- 这是**局部接管**：只把「造舰预算 / 某城忠诚 / 两艘舰」钉成 `Player`，其余（投资、其它舰、
  生产）仍交给 AI。舰 0 去攻击 6 号舰；舰 1 **守卫**舰 2（`attack:false` 的 `target_ship` 就是守卫）。
- **没有 `guard_ship`/`guard` 这个 behavior 类型**——守卫是 `TargetShip{ship:<友舰>, attack:false}`。
- 若想**整体接管**一个势力（所有叶子都归你），才写
  `{"scope":{"factions":[[3,"Player"]]}}`。注意这会让**新造出来的舰默认 Idle**、经济预算也不再
  AI 调——你同时要能扛起这些决策。

### 4.3 behavior 两种写法都认
- **tagged 形式**（就是你从舰的 `order` 字段里看到的）：`{"type":"target_ship","ship":6,"attack":true}`、
  `{"type":"idle"}`、`{"type":"dock","body":9}`。
- **默认枚举形式**：`"Idle"`、`{"TargetShip":{"ship":6,"attack":true}}`。
- 把 `--control` 里的 `order` 原样粘进 diff 即可（`--apply` 会自动归一化）。

### 4.4 关键：**id 是整数身份**
`city`/`building`/`faction`/`ship` 一律用整数 id，不是名字。查 id 用 `--control` 模板、
`cities`/`ships` 的 lazy 表，或 `--meta` 的 `resources`（resource key→中文名同理，WYSIWYG，
状态里的「铁」就是 diff 里的「铁」）。
...
| `--index <DIR>` | 投影成 lean 主流 + 按 id 索引的 lazy 表 + schema.json（planet_xq 读） |
...
- **city/building id 是对象身份**（随扩张/重建会变化），别用名字匹配，也别假设 id 稳定。
- **`metrics.factions` 是 dict（key= faction id 的字符串）**，不是数组；`production` 是每资源
