...
| `[~]` | [控制属性 = 活层](notes/control-live-layers.md) | 三态归属 + 势力级默认指令 + 写值即接管 + **风格活层（doctrine/kiting）**全部落地；§4 四条已裁决；§3.1 记了两轴叶的坑（kit 侧已拦住，引擎侧待裁决）。 | web 两行默认风格（`feature/web-fleet-defaults-style` 在做）；两轴叶初始化语义待裁决 |
