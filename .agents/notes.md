...
| `[x]` | [字段顺序 + **序列化中文名**](notes/field-naming.md)（批 A→D 已落地；第 5 步 = `control.kind` 词表也已统一） | 用户裁决：**Rust 字段按重要性重排 + `#[serde(rename = "中文名")]`**（名字参考 `spec.md`），让 UI **直接用键名当名词、用 `///` 注释（经 schemars 的 `description`）当悬停弹窗**，不再有任何单独的翻译表；`views.json` 的 `label` 退化成"只在需要覆盖时才写"。实测：`src/model` 80 个结构体 / **627 个字段**（85% 已有 `///`），`--schema` 已发 295 条 description。 | **已落地**：实体字段 / 控制叶与 patch 键 / 读面连接键 / `--index` 的 `control.kind` 现在都是中文名词、且**每个词表只有一处声明**（`serde(rename)` / `LEAVES` / `IDENTITY` / `LAZY`）。⚠ 有意保持英文的字符串（`derived_roll` 的盐、`SCHEMA_VERSION`、`temper`/`lone_wolf` 的盐用法、`ShipBehavior` 变体名、表名）见该篇 §8。旧：**待用户拍 §5 的 12 条口径**（spec 原词 vs 短名、`role` 三值、`doctrine`/`kiting` 字段名、`investment_budget` 叫投资还是开发、配置 `game.ron` 的键要不要一起中文化、字段顺序规则、`SCHEMA_VERSION` 23 + digest 换基线）；确认后按批 A→D 做 |
