...
> 知识渠道，见 `tech-system.md`）：`--seed 42 --round 240 --digest 20`（同样取行口径、
> 连跑两次相同、12 行）= `C726F272B461F00A302153C1102385EF4734136ED4F701873D4E0822A9504CBA`。
> 这一步的**前一半（M1：连续化）在同一棵树上逐字节不变**（`657F2DC9…6665`，
> 见 `notes.md` 末尾的快速参考），**后一半（M2）才有意改变世界**。
...
| `[~]` | [科技体系：以 MOND 为干线](notes/tech-system.md)（设计长文） | **道 + 术**：一根连续干线 `mond_control ∈ [0,1]`（**已取代**二元的 `masters` 名单）＋ 挂在它上、**按概率习得**的应用项（未做）；已存在的思潮轴「科学↔技术」就是道与术的分岔阀。掌握度**只连续地改「一次导航尝试的胜算」**：`p = min(1,(eps/(depth×drift×(1−c)))^(1/shape))`、前沿 `r* = 28 + 2/(1−c)`（0→30.0／0.35→31.1／0.7→34.7／0.9→48.0 AU）⇒ 柯伊伯带从「试十几次」变「当月到位」，**没有一步是「解锁了才准去」**。**已落地**：M1 连续化（零回归，digest 逐字节不变）+ M2「**飞船在异常区**」这一条知识渠道（`step_knowledge`：在场强度 `Σ(1+深度×0.25)` → 目标 `1−e^(−p/2)` → 按 0.03 松弛；**道会锈**）+ 观察面三列 + 市场最后两处布尔改连续（丢货 `∝(1−max(双方掌握度))`、承运人=掌握度最高者且抽成 `∝` 它）。 | **M2b（第一优先，待裁决）**：实测 600 回合**全体终值 0**——渠道是「舰队在哪」的被动读数，自动控制**没有理由**把舰留在 28 AU 外（在场率 ≤8%、崇拜教 1%）⇒ 先要**观测动机**（§9 三个候选）。其余：术/`requires`（裁决 6/7）、其余四条渠道（裁决 3）、`SCHEMA_VERSION` 已 13 → 14 |
...
- 纯搬运/拆文件类改动的行为验证：`--seed 42 --round 240 --digest 20` 的 SHA-256 必须逐字节
  ...
  **当前基线（`feature/tech-system-mond`，M2 之后）**：`657F2DC9…6665` → **`C726F272B461F00A302153C1102385EF4734136ED4F701873D4E0822A9504CBA`**
  （12 行、连跑两次相同，见 `notes/tech-system.md` §7：**M1 是零回归**——连续化那一步在同一棵树上
  仍逐字节等于 `657F2DC9…6665`；**M2** 加了「飞船在异常区」这条知识渠道，于是**有意换代**）。
