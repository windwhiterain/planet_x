...
| `[ ]` | [测试全搬的施工图：判据缺什么数据就往序列化里装什么](notes/test-migration-backlog.md) | 用户 2026-10 裁决：**目标是全搬**——判据缺的数据就装进序列化。现状 **120 条 Python 判据 / 208 条 Rust 用例**，剩下的按「缺什么」分四类：**A 数据没序列化**（缺 `ships.cargo`/`cargo_capacity`、逐资源货栈、集货腿 `haul_lanes`；`market_trades` 已够 ⇒ trade 族零新增即可搬）、**B 纯函数**（要 `planet_x --call <fn>` 这类调用路，否则只能留在 Rust）、**C 合成场景**（地基已有：`--save w.json` + Python 改档）、**D 内部契约/错误路径**（`--apply` 报错、`migrate` 分支、中性值表 ⇒ **故意不搬**）。§5 排了六批的执行顺序，每批都要证明装数据那次**行为中性**（digest 逐字不变）。 | 施工图 §1–§5 |
