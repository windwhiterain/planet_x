...
| `[~]` | [科技体系：以 MOND 为干线](notes/tech-system.md)（设计长文） | **道 + 术**：一根连续干线 `mond_control ∈ [0,1]`（**已取代**二元的 `masters` 名单，且「**特权删掉**」⇒ 开局**空表**、没人白拿）＋ 挂在它上、**按概率习得**的应用项（未做）；已存在的思潮轴「科学↔技术」就是道与术的分岔阀。掌握度**只连续地改「一次导航尝试的胜算」**：`p = min(1,(eps/(depth×drift×(1−c)))^(1/shape))`、前沿 `r* = 28 + 2/(1−c)`（0→30.0／0.35→31.1／0.7→34.7／0.9→48.0 AU）⇒ 柯伊伯带从「试十几次」变「当月到位」，**没有一步是「解锁了才准去」**。**已落地**：M1 连续化（零回归，digest 逐字节不变）+ M2「**飞船在异常区**」这一条知识渠道（`step_knowledge`：在场强度 `Σ(1+深度×0.25)` → 目标 `1−e^(−p/2)` → 按 0.03 松弛；**学满 = 持续 48 个够格的回合**（够格 = 在场强度 ≥ 12）⇒ 正好到 1.0 ⇒ **棘轮：到顶永不下降**）+ 观察面三列 + 市场最后两处布尔改连续（丢货 `∝(1−max(双方掌握度))`、承运人=掌握度最高者且抽成 `∝` 它）。 | **当前状态（实测）：干线通着、渠道连着、但没人去拿**——重跑 600 回合**没人到顶**（俄罗斯峰值 0.41、深空运输联盟 0.69，都锈回 0；在场率 ≤7%）⇒ 缺的是**动机**：**§11 观测编队（裁决 a，下一轮，实现地图已写全）**；**§10 = 「把 MOND 好处做大」的七条提案待你挑**（推荐 B1 深空治理／B2 矿藏红利／B3 战斗光环）。其余：术/`requires`（裁决 6/7）、其余四条渠道（裁决 3）、崇拜教失去 master 身份后的补偿（§9.2） |
...
- 纯搬运/拆文件类改动的行为验证：`--seed 42 --round 240 --digest 20` 的 SHA-256 必须逐字节
  ...
  **当前基线（`feature/tech-system-mond`，M2d 之后：特权删掉 + 棘轮）**：`C726F272…4CBA` → **`0E3E760DC20F7F4353343449B5051A92B8D5149C32D29632D9480E7C86589A0C`**
