...
| `[ ]` | [运输：舰船行为 + 集货 + 信誉承包](notes/freight-collection.md) | 公理「**首都即集散地**」定下对外路线；**运输做成舰船真实行为**（`ShipBehavior::Haul` + 产地货栈，非首都产出必须靠船运回）。MOND 已按裁决**概率化**：偏移是伪随机范围，深处**没有进不去的目标**，只是要多试几个回合（实测 伊克西翁 1 回合 / 妊神星 3 / 创神星 8）。承包市场以**信誉**为竞争力（准入资产 + 影子价格 λ 递减的自评闸：低信誉攒信誉接易单、高信誉赚钱接难单）。 | M1 货栈 + M2a 货舱（schema v8，实测**世界 86–88% 的矿冻在产地**）已落地；M2b 运输行为卡在 Q5–Q7（装载语义）；Q1–Q4 待裁决 |
