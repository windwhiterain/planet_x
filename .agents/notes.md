...
| `[~]` | [控制属性 = 活层](notes/control-live-layers.md) | 三态归属（继承/自动/玩家）+ 势力级默认指令 + 写值即接管已落地；§4 四条已裁决（风格两片、`effective` 加、舰级默认归舰船模板）。 | §3 剩余：doctrine/kiting 活层 + 读面 `effective`；`agent-play.md` 跟改 |
| `[~]` | [引擎=数据平面，Python kit=策略平面](notes/engine-data-plane.md) | 引擎只产出 tidy 统计表 + 接受同形状 diff；通配/编制表/统计全归 Python kit（引擎侧通配已否决）。 | `flow`/`pre`/控制面表 + `effective` + `--derived` 未落地 |
...
| `[ ]` | [用 Python 统计地编辑控制面 diff](notes/python-control-authoring.md) | `planet_x_ctl`：把 pandas 的筛选变成合法 diff（通配=展开成显式叶，引擎不加功能）；spec 已定、待开工。 | 套件本体、编制表、示例配方、`agent-play.md` 一节 |
