...
| `[x]` | [太阳的日珥：大片曲面条带 + 本地空间纹理](notes/sun-prominence.md) | 用户拿 SDO/AIA 304Å 参考图要「很多须须喷发出来」的日珥。走了**三条弯路**（等半径球壳 ⇒ 并入日冕体积分 ⇒ 每像素解析求交）**都出不来须**：掠射时视线几乎与日面平行，积分把三十多根针**平均成雾**（不是参数问题）。细针插片版（几千根）密度与细腻只能二选一，被**第二版取代**：**少数大片曲面条带**（`PROM_MAX=5000`，宽 0.075R~0.34R）+ **片元在世界空间场里把带子切成十几~几十根细长日珥**。⚠ 用户纠正过一处：共享的世界空间噪声管的是**带子的扭曲/朝向**，**带子内部的纹理是本地空间**。另有：格点噪声的**方格纸**（新增 `px/noise/{rot,perlin}.glsl`）、配色从参考图**反解** HDR 锚点、几何 **LOD**（画质档 × 日面屏幕半径，按太阳到相机的距离）；**扭** = 涡量沿路径的积分（数据结构 = 一个向量势 Ψ → F=∇×Ψ 无散度 → 旋度沿路径分量；½∫ω·t̂ dl），外加**每片的喷发生命期**。 | 日珥的**弧（loop）**还只是 12% 的小种群；**扭应由「顶点各自采样场」自然产生**（已定量核过：位移 amp=0.30 ⇒ 截面转角 0.84 rad，与手调值一致），见笔记 §4b/§11；插片在**核显**上还没量过 |
...
| `[x]` | [读面第 11 步：左栏改成**分层 UI**（外层总览 → 点进内层详情）](notes/web-read-append.md#8-第-11-步左栏改成分层-ui外层总览--内层详情2026-10已落地)（用户裁决，已落地） | 用户原话「left bar 的UI组织应当尽量用tabs，**不要使用表格**」「不如做成分层的UI，**外层显示总览，点击某个项目进入内层显示详情**」⇒ 7 个页签不变，`layout: layers` 的视图变成两层：外层一行一个项目（名字 + `brief` **声明**出来的少量关键指标），内层是那一条记录的「名字 → 值」逐条行（声明列按人工顺序在前，**其余引擎字段追加在末尾**——铁律 R 换落点，没退），顶部面包屑 `势力 › 中国`（根那一节也能点）+「← 返回总览」，返回时**滚动位置原样还回来**。只有一条记录的视图（战局/价格/全局作用域）写 `single` + 非空理由（直接显示详情，不造只有一行的列表）。**引擎一行未动**（只动 `specview.js`/`views.json`/`app.js`/`style.css`/`index.html` + 两把量具）。 | 新判据 `g4_spec.py` **§10a/§10b/§10c**（声明自洽 / 每个总览行都能进详情层（假 DOM 里**真渲染**走一遍）/ 详情层字段集合 == 该实体全部字段=§8a 的渲染落点）；§8a **换落点到详情层**（实测 15 表/216 记录/195 字段 = 87 声明 + 103 追加 + 5 不看），§8b/§9 一个字没改、照旧绿。反向验证 `_g4_negative.py` **㊱–㊶**（进入路径断 / `brief` 不命中 / 详情层不追加残差（§8a 仍绿）/ `single` 理由空白 / 多记录表谎称 `single` / 空对象幽灵行）⇒ **46 个注入错全咬住**。实机（:3000，本 worktree）：势力页 9 行总览（无表格、无横滚）、中国详情 30 行（末尾 7 条追加）/面包屑与返回、返回后 `scrollTop` 148→148、追加字段 hover 弹窗、123 行逐个进出 + 134 次 hover ⇒ `window.__errs == []`。 | **拿不准**：`renderTable`/`renderTimeline`/`renderCards` 留在求值器里（声明层已不可达）——删它们会一起删掉 §8b/§9 依赖的接线标记 `sv-th-auto`/`sv-td-auto`/`autoTh`，要收得连量具口径一起改，留待裁决；`events` 的总览行只能放 4 条固定载荷列（多态变体，多数行显示 `—`）；`offers` 的 key=`seller` 不唯一（要引擎给复合身份） |
