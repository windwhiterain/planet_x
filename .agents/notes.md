...
| `[ ]` | [科技体系：以 MOND 为干线](notes/tech-system.md)（设计长文） | **道 + 术**：一根连续干线 `mond_control ∈ [0,1]`（取代二元的 `masters`）＋ 挂在它上、**按概率习得**的应用项；已存在的思潮轴「科学↔技术」就是道与术的分岔阀。掌握度**只连续地改「一次导航尝试的胜算」**：`p = min(1,(eps/(depth×drift×(1−c)))^(1/shape))`、前沿 `r* = 28 + 2/(1−c)`（0→30.0／0.35→31.1／0.7→34.7／0.9→48.0 AU）⇒ 柯伊伯带从「试十几次」变「当月到位」，**没有一步是「解锁了才准去」**。知识五渠道（在场/开采/残骸/条约/沿接触扩散）、**道会锈而术不锈**、术靠抽签习得；六个空坑（战斗光环/矿藏红利/科技扩散/舰级/结构/舰种）全挂在这一根轴上。 | **未实现**（只有设计）；§8 有**九处待裁决**（首轮六问 + 门槛形态/阶梯是否闸门/活死知识）；落地分 M1（零回归，digest 逐字节不变）→ M2（行为改变）→ M3 |
