...
| `[ ]` | [运输：舰船行为 + 集货 + 信誉承包](notes/freight-collection.md) | 公理「**首都即集散地**」定下对外路线；**运输做成舰船真实行为**（`ShipBehavior::Haul` + 产地货栈，非首都产出必须靠船运回）。MOND 已按裁决**概率化**：偏移是伪随机范围，深处**没有进不去的目标**，只是要多试几个回合（实测 伊克西翁 1 回合 / 妊神星 3 / 创神星 8）。承包市场以**信誉**为竞争力（准入资产 + 影子价格 λ 递减的自评闸：低信誉攒信誉接易单、高信誉赚钱接难单）。 | M1 货栈已落地；M2 起的运输行为/承包市场待做；先裁决 Q1–Q4 |
