...
| `[~]` | [国内市场：资源预算 → 货币预算 → 真实交易](notes/domestic-market.md) | 上层投放资源 + 拨钱，下层按 recipe 在市场买；价格逐回合反馈，替代每回合中央 allocator。第一版默认 `enabled=false`，开发/建造先接入，福利仍走旧路径。 | 福利并轨、货币控制叶、城际交易、长局 A/B |
...
| `[ ]` | [字段顺序 + **序列化中文名**](notes/field-naming.md)（设计待裁决） | 用户裁决：**Rust 字段按重要性重排 + `#[serde(rename = "中文名")]`**（名字参考 `spec.md`），让 UI **直接用键名当名词、用 `///` 注释（经 schemars 的 `description`）当悬停弹窗**，不再有任何单独的翻译表；`views.json` 的 `label` 退化成"只在需要覆盖时才写"。实测：`src/model` 80 个结构体 / **627 个字段**（85% 已有 `///`），`--schema` 已发 295 条 description。 | **待用户拍 §5 的 12 条口径**（spec 原词 vs 短名、`role` 三值、`doctrine`/`kiting` 字段名、`investment_budget` 叫投资还是开发、配置 `game.ron` 的键要不要一起中文化、字段顺序规则、`SCHEMA_VERSION` 23 + digest 换基线）；确认后按批 A→D 做 |
...
| `[~]` | [测试全搬的施工图：判据缺什么数据就往序列化里装什么](notes/test-migration-backlog.md) | 用户 2026-10 裁决：**目标是全搬**——判据缺的数据就装进序列化。现状 **164 条 Python 判据 / 196 条 Rust 用例**（g1 43 / g2 77 / g3 26 / g4 18），剩下的按「缺什么」分四类：**A 数据没序列化**（第 1–3 批已落地：`ships.载货`/`cargo_capacity`、逐资源货栈 `depots`；集货腿 `haul_lanes` **否决**——§5.4）、**B 纯函数**（第 5 批 `planet_x --call <fn>` 已落地）、**C 合成场景**（第 6 批已落地：`_harness.scenario_apply` 走引擎自己的 `--apply` 拨控制叶）、**D 内部契约/错误路径**（`--apply` 报错、`migrate` 分支、中性值表 ⇒ **故意不搬**，清单在 §4）。§5 排了七批的执行顺序，每批都要证明装数据那次**行为中性**（digest 逐字不变）。 | 施工图 §0.1 / §5.6 / §6.5 |
