...
| `[ ]` | [舰船设计图](notes/ship-blueprint.md)（设计长文） | 非控制属性（面板/选装/造价）放在**建造单位**上作为出厂快照的设计图；三处旧账的同一个解。 | 全部；文内 §3 五条语义待用户裁决 |
...
| `[~]` | [控制属性 = 活层](notes/control-live-layers.md) | 三态归属（继承/自动/玩家）+ 势力级默认指令 + 写值即接管已落地；doctrine/kiting 活层与按舰级默认未做。 | §3 剩余；§4 四条待用户裁决 |
...
| `[ ]` | [用 Python 统计地编辑控制面 diff](notes/python-control-authoring.md) | 读侧 `planet_xq` 已有，但投影里**没有控制面**；要一个能"筛 + 产出合法 diff"的 `planet_x_ctl`。 | 全部；文内 §3 三条待用户裁决 |
