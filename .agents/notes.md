...
| `[ ]` | [面向人类的读面：声明式组织点 + 通用兜底](notes/web-human-views.md)（设计长文） | 现状核实「**读面通用但无组织，写面有组织但硬编码**」＋实机量出的缺口（118 KB/帧、`MAX_COLS` 静默丢列、推进 20 回合零反馈）；设计 = 声明式 `views.json` + 通用求值器 + **铁律 R「只能整理，不能隐藏」**（残差 = 集合差 ⇒ 扩展自动可见）。 | **等裁决 §9 的 D1–D6**；实现顺序 M1–M4 见该篇 §10 |
