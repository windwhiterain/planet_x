...
| `[ ]` | [测试与二进制解耦：共享轨迹 + 数据级断言](notes/test-decoupled-suite.md) | **方案，一行代码没动。**用户提的方向：*「测试应该和游戏二进制解耦，直接测跑出来的数据，这样不管多少测试只用编译一遍」/「也许可以是 python 测试」/「甚至轨迹可以复用」*。实测成本模型：改一个文件后 test 档重建 **46.7 s**、release 档 **105.4 s**，而全档真跑只有 **26.5 s**（⇒ 时间全在编译，B5 一秒没加）；四条长局跑的是**完全相同**的 `seeds=[1..=5,7,11] × 1000 回合`（同一批 2.8 万回合重算四遍）；8 个测试二进制各链一遍整个 crate。三级方案：**Stage 1** 共享轨迹（只动测试内部，全档 → ~10 s）、**Stage 2** pytest + 投影缓存（键 = 二进制哈希+seed+回合数，代码一改自动失效）、**Stage 3** 档位（Stage 2 后 test 档可回 O0，7.6 s 重编）。另记了「短档 debug / 长档优化」的 A/B（内循环 O0 赢 27 s vs 51 s，但长档 O2 快 4×，且切档要重编依赖 51.6 s ⇒ **不建议现在做**，Stage 2 会化解它）。 | §6「什么必须留在 Rust」的两栏对账；§9 四个待裁决（runner / 先搬哪族 / 用例粒度 / 要不要先做 Stage 1） |
